...
export const prepareProjectScriptUpdate = Effect.fn("prepareProjectScriptUpdate")(function* (
  current: ServerSettings,
  patch: ServerSettingsPatch,
  settingsPath: string,
) {
  const { projectScriptUpdate: update, ...rest } = patch;
  if (update === undefined) return rest;
  if (!current.projectSettingsFolded) {
    return yield* new ServerSettingsError({
      settingsPath,
      operation: "project-actions-unavailable",
      cause:
        "Project settings have not been folded. Repair the settings file before editing actions.",
    });
  }
  const overrides = current.projectSettingsOverrides[update.projectId] ?? {};
  const scripts = overrides.defaultProjectScripts ?? current.defaultProjectScripts;
  if (!Equal.equals(scripts, update.expectedScripts)) {
    return yield* new ServerSettingsError({
      settingsPath,
      operation: "project-actions-conflict",
      cause: "Project actions changed. Read them again before retrying.",
    });
  }
  return {
    ...rest,
    projectSettingsOverrides: {
      ...rest.projectSettingsOverrides,
      [update.projectId]: {
        ...overrides,
        ...rest.projectSettingsOverrides?.[update.projectId],
        defaultProjectScripts: update.scripts,
      },
    },
  };
});
        ...
        writeSemaphore.withPermits(1)(
          Ref.get(currentSettingsRef).pipe(
            Effect.flatMap((currentSettings) =>
              prepareProjectScriptUpdate(currentSettings, patch, "test").pipe(
                Effect.map((prepared) => applyServerSettingsPatch(currentSettings, prepared)),
              ),
            ),
          ...
          if (usageSource.managementKey.length > 0 || !usageSource.managementKeyRedacted) {
            if (usageSource.managementKey.length > 0) {
              changes.push({
                kind: "write",
                secretName,
                value: textEncoder.encode(usageSource.managementKey),
                providerInstanceId: instanceId,
              });
  ...
  const updateSettings = (
    ...
    writeSemaphore.withPermits(1)(
      Effect.gen(function* () {
        ...
        const prepared = yield* prepareProjectScriptUpdate(current, patch, settingsPath);
        const updated = applyServerSettingsPatch(current, prepared);
