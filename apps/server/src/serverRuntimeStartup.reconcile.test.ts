...
const makeProviderService = (liveThreadIds: ReadonlyArray<ThreadId> = []) =>
  ({
    ...
    stopSessionIfUnchanged: () => Effect.die("unused"),
    ...
    Effect.gen(function* () {
      ...
      yield* runReconciliation({
        ...
        directory: {
          ...
          upsert: (binding) =>
            Effect.sync(() => {
              bindings.set(binding.threadId, withBindingMetadata(binding));
...
for (const preparedStatus of [
  ...
  it.effect(`recovers again if startup exits with a prepared ${preparedStatus} session`, () =>
    Effect.gen(function* () {
      ...
      const sends: ProviderSendTurnInput[] = [];
      let binding = withBindingMetadata({
        threadId: thread.id,
        provider: ProviderDriverKind.make("codex"),
        providerInstanceId,
        status: "running",
      ...
      const input = {
        ...
        directory: {
          ...
          upsert: (next: ProviderSessionDirectory.ProviderRuntimeBinding) =>
            Effect.gen(function* () {
              binding = withBindingMetadata(next);
      ...
      [
        {
          status: "starting",
          activeTurnId: null,
          expectedSession: { status: "running", activeTurnId: turnId },
        },
        {
          status: "error",
          activeTurnId: null,
          expectedSession: { status: "starting", activeTurnId: null },
        },
      ],
    );
    assert.equal(binding.status, "stopped");
