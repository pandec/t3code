...
export const reconcileProviderSessions = Effect.gen(function* () {
  ...
  for (const thread of orphanedThreads) {
    ...
    const settleAsError = (
      lastError: string,
      // The state this writer last observed. The synchronous callers observed
      // the startup projection read; the asynchronous continuation-failure
      // path observed the "starting" state it wrote itself. A mismatch means
      // another writer (a user restart, a steer) moved the session on, and the
      // stale error spread must not clobber it.
      observedSession: ThreadSessionExpectation = {
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          // A compare-and-set rejection surfaces through the retry/log path
          // below: the session moved on since this pass observed it and the
          // newer state carries the truth, so the stale error spread is
    ...
    if (
      ...
      const prepared = yield* Effect.gen(function* () {
        ...
        yield* orchestrationEngine.dispatch({
          ...
          createdAt: resumedAt,
          // Anything that moved the session since the startup read (a user
          // restart racing this pass) owns it now; a stale rewrite here would
          // revive superseded state, so fail the preparation instead.
          expectedSession: { status: session.status, activeTurnId: session.activeTurnId },
        });
