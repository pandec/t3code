...
describe("audio asset byte ranges", () => {
  ...
  it.effect("streams audio ranges from guarded descriptors", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "t3-guarded-audio-range-" });
      ...
      const response = HttpServerResponse.toWeb(
        yield* assetFileResponse(
          { path: canonicalPath, file, mimeType: "audio/mpeg" },
          "bytes=2-5",
        ),
      );
      expect(response.status).toBe(206);
      ...
      expect(response.headers.get("cache-control")).toBe("private, no-store");
      expect(yield* Effect.promise(() => response.text())).toBe("2345");
    }).pipe(Effect.provide(fileResponseLayer)),
  );
