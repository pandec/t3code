...
async function startFakeDesktop(input: {
  ...
  readonly stateSubdirectory?: "dev-packaged" | "userdata" | "dev";
...
describe("t3 app", () => {
  ...
  it.effect("prefers the packaged Dev desktop even when T3CODE_HOME is set", () =>
    withTempDirectory("t3-app-dev-packaged-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "t3-home");
        const packagedDevelopment = yield* fakeDesktop({
          baseDir,
          stateSubdirectory: "dev-packaged",
        });
        const installed = yield* fakeDesktop({ baseDir });
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });

        yield* runCli(["app"], { T3CODE_HOME: baseDir });

        expect(packagedDevelopment.received).toHaveLength(1);
        expect(installed.received).toHaveLength(0);
  ...
  it.effect("never searches development state directories with --base-dir", () =>
    withTempDirectory("t3-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(packagedDevelopment.received).toHaveLength(0);
        expect(development.received).toHaveLength(0);
      }).pipe(Effect.scoped),
    ),
  );
