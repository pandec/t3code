import { ComposerContextId, CommandId, MessageId, ThreadId, TurnId } from "@t3tools/contracts";
...
layer("ProjectionThreadMessageRepository", (it) => {
  ...
  it.effect("persists structured context and keeps it across updates without context", () =>
    Effect.gen(function* () {
      ...
      const destinationThreadId = ThreadId.make("thread-context-fork");
      ...
      assert.isUndefined(forked[0]?.context);
      assert.isUndefined(forked[0]?.attachments);
    }),
  );
