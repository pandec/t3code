...
export const make = Effect.gen(function* () {
    ...
    projections.getShellSnapshot().pipe(
      ...
      Effect.map(({ refinedKinds, snapshot }) => {
        ...
        for (const project of snapshot.projects) {
          ...
          if (api !== null) {
            const candidate = { project, api, repository, host } satisfies SupportedProject;
  ...
  const requireProject = (
    ref: PullRequestRef,
    options: { readonly verifyLive?: boolean } = {},
  ): Effect.Effect<SupportedProject, PullRequestError> =>
    listWorkspaceProjects({ projectId: ref.projectId }).pipe(
      Effect.flatMap(
        ({ supported, checkoutsByRepository }): Effect.Effect<SupportedProject, PullRequestError> =>
          Effect.gen(function* () {
            ...
            if (options.verifyLive !== true) return match;

            const expectedCanonicalKey = match.project.repositoryIdentity?.canonicalKey;
            if (expectedCanonicalKey !== undefined) {
              const siblings =
                checkoutsByRepository.get(listCursorKey(match.host, match.repository)) ?? [];
              const candidates = [
                match,
