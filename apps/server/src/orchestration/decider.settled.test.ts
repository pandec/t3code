...
it.layer(NodeServices.layer)("settled thread decider", (it) => {
  ...
  it.effect("settling an indefinitely snoozed thread also wakes it", () =>
    Effect.gen(function* () {
      const result = yield* decideOrchestrationCommand({
        command: {
          type: "thread.settle",
          commandId: CommandId.make("cmd-settle-snoozed-indefinitely"),
          threadId: ThreadId.make("thread-1"),
        },
        // "Until I wake it" carries a null wake time, so snoozedAt is the only
        // evidence the thread is snoozed at all.
        readModel: makeReadModel(null, null, null, [], [], {
          snoozedUntil: null,
          snoozedAt: SETTLED_AT,
        }),
      });
      const events = Array.isArray(result) ? result : [result];
      expect(events.map((entry) => entry.type)).toEqual(["thread.settled", "thread.unsnoozed"]);
  ...
  it.effect("rejects automatic settlement for an indefinitely snoozed thread", () =>
    Effect.gen(function* () {
      const error = yield* decideOrchestrationCommand({
        command: {
          type: "thread.auto-settle",
          commandId: CommandId.make("cmd-auto-settle-snoozed-indefinitely"),
          threadId: ThreadId.make("thread-1"),
          snapshotSequence: 0,
          settledAt: SETTLED_AT,
        },
      ...
      expect(error).toMatchObject({
        ...
        commandType: "thread.auto-settle",
      });
    }),
  );
