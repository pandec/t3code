 ...
 * anchor timestamp (`projection_turns.requested_at`) and turn id.
...
export function decodeThreadDetailPageCursor(encoded: string): ThreadDetailPageCursor | null {
  ...
  // Empty strings are valid boundary values, not malformed input: the sentinel
  // anchor "" sorts before every real `requested_at` and correctly ends the
  // walk, and the turn key is "" for a turn row with a null turn_id.
