...
const makeHarness = Effect.fn("makeThreadSettlementHarness")(function* (options: HarnessOptions) {
  ...
  const settingsReadCount = yield* Ref.make(0);
  const settingsReads = yield* Queue.unbounded<number>();
  ...
  const serverSettings = ServerSettingsService.of({
    start: Effect.void,
    ready: Effect.void,
    getSettings: Ref.updateAndGet(settingsReadCount, (count) => count + 1).pipe(
      Effect.tap((count) => Queue.offer(settingsReads, count)),
