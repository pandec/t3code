...
describe("ThreadSettlementReactor", () => {
  ...
  it.effect("skips gated sweeps and runs immediately when automatic settlement is enabled", () =>
    Effect.scoped(
      Effect.gen(function* () {
        yield* TestClock.setTime(Date.parse(NOW));
        const fixture = yield* makeHarness({
          snapshot: makeSnapshot([makeThread("gated-thread", { branch: "saved-feature" })]),
          settings: {
            ...DEFAULT_SERVER_SETTINGS,
        ...
        yield* Effect.gen(function* () {
          ...
          yield* fixture.updateSettings({ threadAutoSettleEnabled: true });
          yield* Queue.take(fixture.snapshotReads);
          yield* reactor.drain;
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const rawConfig = yield* ServerConfig;
          const canonicalBase = yield* fs.realPath(rawConfig.baseDir);
          const config = {
            ...rawConfig,
            baseDir: canonicalBase,
            worktreesDir: path.join(
              canonicalBase,
              path.relative(rawConfig.baseDir, rawConfig.worktreesDir),
            ),
            browserArtifactsDir: path.join(
              canonicalBase,
              path.relative(rawConfig.baseDir, rawConfig.browserArtifactsDir),
            ),
            logsDir: path.join(canonicalBase, path.relative(rawConfig.baseDir, rawConfig.logsDir)),
          };
          ...
          if (protection === "background" || protection === "deleted-background") {
                  ...
                  getShellSnapshot: () =>
                    Deferred.succeed(snapshotRead, undefined).pipe(
                      Effect.andThen(
                        Effect.sync(() => {
                          snapshotReads++;
                          if (protection === "late-background" && snapshotReads > 1) {
                            liveness.recordTaskLiveness({
                              threadId: thread.id,
                              taskId: "live",
                              taskType: "agent",
                          ...
                          if (
                            ...
                            (protection === "late-pending-switch" && snapshotReads > 1)
                          ) {
                            threads.push(
                              makeThread("pending-switch-owner", {
                                worktreeSwitch: {
