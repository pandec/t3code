...
it.layer(NodeServices.layer)("pinned thread decider", (it) => {
  ...
  it.effect("pinning an indefinitely snoozed thread also wakes it", () =>
    Effect.gen(function* () {
      const event = yield* decideOrchestrationCommand({
        command: {
          type: "thread.pin",
          commandId: CommandId.make("cmd-pin-indefinitely-snoozed"),
          threadId: ThreadId.make("thread-1"),
        },
        readModel: makeReadModel({ snoozedAt: PINNED_AT, snoozedUntil: null }),
      });
      const events = Array.isArray(event) ? event : [event];
      expect(events.map((entry) => entry.type)).toEqual(["thread.pinned", "thread.unsnoozed"]);
    }),
