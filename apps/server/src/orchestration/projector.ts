    ...
    case "thread.deleted":
      return decodeForEvent(ThreadDeletedPayload, event.payload, event.type, "payload").pipe(
        Effect.map((payload) => {
          ...
          // A tombstoned thread must not show up as linked to a pull request; the
          // SQL pipeline drops its link rows, so the in-memory model does too.
          return {
            ...nextBase,
            threads: updateThread(nextBase.threads, payload.threadId, {
              ...(thread === undefined ? {} : pullRequestsPatch(thread, [], nextBase.projects)),
              deletedAt: payload.deletedAt,
              updatedAt: payload.deletedAt,
            }),
          };
        ...
        Effect.map((payload) => {
          ...
          return {
            ...
            threads: updateThread(nextBase.threads, payload.threadId, {
              ...
              ...(payload.customGroupId !== undefined
