...
const decodeRecentArchivedThreads = Schema.decodeUnknownEffect(
  OrchestrationGetRecentArchivedThreadsResult,
);
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
    ...
    execute: () =>
      sql`
        SELECT
          ...
          snoozed_until_turn_id AS "snoozedUntilTurnId",
          pinned_at AS "pinnedAt",
          pin_order_key AS "pinOrderKey",
          active_order_key AS "activeOrderKey",
          custom_group_id AS "customGroupId",
  ...
  // its ORDER BY uses raw columns so the 045_ProjectionTurnsKeysetIndex
  ...
  // never be reached by the client and would park the page forever. Both
  // read THREAD_DETAIL_EVENT_TYPES so the lists cannot drift. Served
  ...
  // are metadata-scale.
  // Shares `threadMessageArtifactSelect` with the unwindowed query so a turn
  // page carries the same message artifacts (input origin, generated summary,
  // speech) as a full snapshot — a windowed client must not silently lose them.
...
pending_approval_requests AS (
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                snoozedUntilTurnId: row.snoozedUntilTurnId ?? null,
                pinnedAt: row.pinnedAt,
                pinOrderKey: row.pinOrderKey ?? null,
                activeOrderKey: row.activeOrderKey ?? null,
                customGroupId: row.customGroupId ?? null,
  ...
  const getRecentArchivedThreads: ProjectionSnapshotQueryShape["getRecentArchivedThreads"] = (
    input,
  ) =>
    sql
      .withTransaction(
        Effect.gen(function* () {
          const [threadRows, countRow, stateRows] = yield* Effect.all([
            listRecentArchivedThreadRows(input).pipe(
              Effect.mapError(
                toPersistenceSqlOrDecodeError(
                  "ProjectionSnapshotQuery.getRecentArchivedThreads:listThreads:query",
                  "ProjectionSnapshotQuery.getRecentArchivedThreads:listThreads:decodeRows",
                ),
              ),
            ),
            getArchivedThreadCount(input).pipe(
              Effect.mapError(
                toPersistenceSqlOrDecodeError(
                  ...
                  "ProjectionSnapshotQuery.getRecentArchivedThreads:countThreads:decodeRow",
                ),
              ),
            ),
            listProjectionStateRows(undefined).pipe(
              Effect.mapError(
                toPersistenceSqlOrDecodeError(
                  "ProjectionSnapshotQuery.getRecentArchivedThreads:listProjectionState:query",
                  "ProjectionSnapshotQuery.getRecentArchivedThreads:listProjectionState:decodeRows",
                ),
              ),
            ),
          ]);
          const relatedRows = yield* Effect.forEach(
            threadRows,
            (row) =>
              Effect.all([
                getLatestTurnRowByThread({ threadId: row.threadId }).pipe(
                  Effect.mapError(
                    toPersistenceSqlOrDecodeError(
                      "ProjectionSnapshotQuery.getRecentArchivedThreads:getLatestTurn:query",
                      "ProjectionSnapshotQuery.getRecentArchivedThreads:getLatestTurn:decodeRow",
                    ),
                  ),
                ),
                getThreadSessionRowByThread({ threadId: row.threadId }).pipe(
                  Effect.mapError(
                    toPersistenceSqlOrDecodeError(
                      "ProjectionSnapshotQuery.getRecentArchivedThreads:getSession:query",
                      "ProjectionSnapshotQuery.getRecentArchivedThreads:getSession:decodeRow",
                    ),
                  ),
                ),
                listThreadPullRequestRowsByThread({ threadId: row.threadId }).pipe(
                  Effect.mapError(
                    toPersistenceSqlOrDecodeError(
                      "ProjectionSnapshotQuery.getRecentArchivedThreads:listPullRequests:query",
                      "ProjectionSnapshotQuery.getRecentArchivedThreads:listPullRequests:decodeRows",
                    ),
                  ),
                ),
              ]),
          ...
          // The legacy single link is derived per project identity, like every other shell read.
            ...
            yield* Effect.forEach(
              ...
              (projectId) =>
                getProjectShellById(projectId).pipe(
                  Effect.map(
                    (project) =>
                      [projectId, Option.getOrNull(project)?.repositoryIdentity ?? null] as const,
                  ),
                ),
              { concurrency: "unbounded" },
            ),
          );
          return { threadRows, countRow, stateRows, relatedRows, repositoryIdentities };
        }),
      )
      .pipe(
        Effect.flatMap(({ threadRows, countRow, stateRows, relatedRows, repositoryIdentities }) => {
          let updatedAt: string | null = null;
          for (const row of threadRows) {
            updatedAt = maxIso(updatedAt, row.updatedAt);
          }
          ...
          const threads = threadRows.map((row, index): OrchestrationThreadShell => {
            const [latestTurnRow, sessionRow, pullRequestRows] = relatedRows[index]!;
            if (Option.isSome(latestTurnRow)) {
              updatedAt = maxIso(updatedAt, latestTurnRow.value.requestedAt);
              if (latestTurnRow.value.startedAt !== null) {
                updatedAt = maxIso(updatedAt, latestTurnRow.value.startedAt);
              }
              if (latestTurnRow.value.completedAt !== null) {
                updatedAt = maxIso(updatedAt, latestTurnRow.value.completedAt);
              }
            }
            ...
            return {
              ...
              customGroupId: row.customGroupId,
              branchPullRequest: row.branchPullRequest,
              archiveRequest: row.archiveRequest ?? null,
              worktreeSwitch: row.worktreeSwitch ?? null,
              titleRegeneration: mapTitleRegeneration(row),
              ...
              ...mapThreadPullRequests(
                ...
                row.projectId,
                repositoryIdentities.get(row.projectId) ?? undefined,
              ),
          ...
          return decodeRecentArchivedThreads({
            snapshotSequence: computeSnapshotSequence(stateRows),
            threads,
            totalArchivedCount: countRow.totalArchivedCount,
            updatedAt: updatedAt ?? "1970-01-01T00:00:00.000Z",
          }).pipe(
            Effect.mapError(
              toPersistenceDecodeError(
                "ProjectionSnapshotQuery.getRecentArchivedThreads:decodeResult",
              ),
            ),
          );
        }),
        Effect.mapError((error) =>
          isPersistenceError(error)
            ? error
            : toPersistenceSqlError("ProjectionSnapshotQuery.getRecentArchivedThreads:query")(
                error,
              ),
        ),
      );
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        snoozedUntilTurnId: threadRow.value.snoozedUntilTurnId ?? null,
        pinnedAt: threadRow.value.pinnedAt,
        pinOrderKey: threadRow.value.pinOrderKey ?? null,
        activeOrderKey: threadRow.value.activeOrderKey ?? null,
        customGroupId: threadRow.value.customGroupId ?? null,
    ...
    sql
      .withTransaction(
        Effect.gen(function* () {
          ...
          // Probe for an older turn BEFORE resolving bounds: the answer decides
          // whether this is the thread's terminal page, which in turn decides
          // the lower bound (see below). The probe reports any turn strictly
          // older than `oldest` — a run of non-user turns still surfaces,
          // because `user_turns_seen` stays 0 for them — so an empty result
          // means no older turn exists at all.
          const hasMore =
            oldest !== undefined &&
          ...
          //
          // On the TERMINAL page (no older turn exists) the lower bound is
          // widened to unbounded. Rows with no turn linkage are bounded by
          // `created_at >= minAnchorAt`, so leaving the bound at the oldest
          // turn's `requested_at` would strand every turnless row older than
          // the thread's first turn — imported history (turnId null, original
          // timestamps) and pre-first-turn activities — in no page at all,
          // while `hasMore: false` simultaneously declares history complete.
          // Widening adds no turns: the probe just proved none exist below.
          const includesOldestTurn = oldest !== undefined && !hasMore;
          const bounds: ThreadDetailBounds | undefined =
            oldest === undefined && cursor === null
              ? undefined
              : {
                  minAnchorAt: includesOldestTurn ? "" : (oldest?.anchorAt ?? ""),
                  minTurnKey: includesOldestTurn ? "" : (oldest?.turnKey ?? ""),
                  beforeAnchorAt: cursor?.beforeAnchorAt ?? ANCHOR_UNBOUNDED,
                  beforeTurnKey: cursor?.beforeTurnId ?? "",
                };
