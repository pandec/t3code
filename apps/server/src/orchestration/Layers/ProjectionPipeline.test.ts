...
engineLayer("OrchestrationProjectionPipeline via engine dispatch", (it) => {
  ...
  it.effect("re-creating a deleted thread id starts from an empty projection", () =>
    Effect.gen(function* () {
      ...
      // Fork per-message artifacts: no cascading foreign key, and a
      // superseded thread.deleted skips their cleanup on replay, so the
      // thread.created reset must clear them too. Seeded after the delete to
      // model that replay gap, so only the post-recreate loop checks them.
      const perThreadTablesAfterRecreate = [
        ...perThreadTables,
        "projection_message_speech",
        "projection_message_summary",
      ];
      ...
      // Model the replay path where a superseded thread.deleted skips the
      // fork's artifact cleanup: old-incarnation speech/summary rows are
      // still on disk when the reused thread id is created again.
      yield* sql`
        INSERT INTO projection_message_speech (
          message_id, thread_id, speech_id, transcript, mime_type, size_bytes,
          source_text_hash, script_recipe_hash, voice_id, tts_model, created_at
        ) VALUES (
          'message-retry-1', ${threadId}, 'speech-retry-1', 'first attempt',
          'audio/mpeg', 3, 'hash', 'recipe', 'voice', 'tts', ${createdAt}
        )
      `;
      yield* sql`
        INSERT INTO projection_message_summary (
          ...
          'message-retry-1', ${threadId}, 'a summary', 'hash', 'recipe',
          'model-hash', ${createdAt}
        )
      `;
      ...
      for (const table of perThreadTablesAfterRecreate) {
