...
interface OrchestrationSystemOptions {
...
function makeOrchestrationLayer(options: OrchestrationSystemOptions = {}) {
  const persistence = options.databasePath
  ...
  return Layer.mergeAll(
    ...
    Layer.provide(
      options.repositoryIdentityResolver !== undefined
        ? Layer.succeed(
            RepositoryIdentityResolver.RepositoryIdentityResolver,
            options.repositoryIdentityResolver,
          )
        : options.repositoryIdentity === undefined
          ? RepositoryIdentityResolver.layer
          : Layer.succeed(RepositoryIdentityResolver.RepositoryIdentityResolver, {
              resolve: () => Effect.succeed(options.repositoryIdentity ?? null),
            }),
    ),
...
async function createOrchestrationSystem(options: OrchestrationSystemOptions = {}) {
...
describe("OrchestrationEngine", () => {
    ...
    async (status) => {
      ...
      try {
        ...
        const response = {
          ...
          threadId,
          requestId,
          // Clients send a multi-select answer as the chosen option values.
          answers: { "0": "pnpm", "1": "Example", "2": ["lint", "tests"] },
        ...
        await expect(
          system.run(
            system.engine.dispatch({
              ...response,
              commandId: CommandId.make("empty-multi-select"),
              answers: { "0": "pnpm", "1": "Example", "2": [] },
            }),
          ),
        ).rejects.toThrow("Answer each question before sending.");
    ...
    const layer = OrchestrationEngineLive.pipe(
      Layer.provide(
        Layer.succeed(ProjectionSnapshotQuery, {
          ...
          getThreadMessagePage: () => Effect.succeed(Option.none()),
  ...
  effectIt.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // The settle-cleanup session stop must not tear down live background
        // work either: a settled thread can still run subagents or monitors.
        backgroundLiveness.recordTaskLiveness({
          threadId: liveThreadId,
          taskId: "task-settle-cleanup-guard",
          taskType: "subagent",
          status: undefined,
          kind: "started",
        });
        const cleanupStopError = yield* engine
          .dispatch({
            type: "thread.session.stop",
            commandId: CommandId.make("cmd-settle-cleanup-stop-live"),
            threadId: liveThreadId,
            createdAt: now(),
            onlyIfSettled: true,
          })
          .pipe(Effect.flip);
        expect(cleanupStopError._tag).toBe("OrchestrationCommandInvariantError");
        backgroundLiveness.clearThreadLiveness(liveThreadId);
      }).pipe(Effect.provide(makeOrchestrationLayer())),
  );
  ...
  it.each([
    ...
    "manual",
    "checkpoint-failed",
  ] as const)("persists a deferred archive across restart and handles %s", async (outcome) => {
    const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "t3-archive-"));
    const databasePath = NodePath.join(directory, "state.sqlite");
    let system = await createOrchestrationSystem({ databasePath });
    const threadId = ThreadId.make("deferred-archive");
    const projectId = ProjectId.make("archive-project");
    const turnId = TurnId.make("target-turn");
    const requestId = CommandId.make("schedule-archive");
    ...
    try {
      ...
      expect((await system.readModel()).threads[0]?.archivedAt).toBeNull();
      await expect(
        dispatch({
          type: "thread.archive.execute",
          commandId: CommandId.make("too-early"),
          threadId,
          requestId,
        }),
      ...
      await system.dispose();
      system = await createOrchestrationSystem({ databasePath });
      expect((await system.readModel()).threads[0]?.archiveRequest?.turnId).toBe(turnId);
      if (outcome === "manual") {
        await dispatch({ type: "thread.archive", commandId: CommandId.make("manual"), threadId });
      } else if (outcome === "checkpoint-failed") {
        await dispatch({
          type: "thread.activity.append",
          commandId: CommandId.make("capture-failed"),
          threadId,
          createdAt: now(),
          activity: {
        ...
        await dispatch({
          ...
          threadId,
          session: session(
            outcome === "new-turn" ? "running" : outcome === "completed" ? "ready" : outcome,
            outcome === "new-turn" ? TurnId.make("new-turn") : null,
          ),
          createdAt: now(),
        });
        if (outcome === "completed") {
          await expect(
            dispatch({
              type: "thread.archive.execute",
              commandId: CommandId.make("before-checkpoint"),
              threadId,
              requestId,
            }),
          ...
          // The engine's startup snapshot deliberately omits checkpoints.
          await system.dispose();
          system = await createOrchestrationSystem({ databasePath });
        }
        await dispatch({
      ...
      if (outcome === "completed") {
        ...
        await expect(
          dispatch({
            ...
            commandId: CommandId.make("unsafe-reopen"),
            threadId,
          }),
        ...
        await expect(
          dispatch({
            type: "thread.create",
            commandId: CommandId.make("attach-during-cleanup"),
        ...
        if (outcome === "manual") expect(thread?.archivedAt).not.toBeNull();
        else expect(thread?.archivedAt).toBeNull();
        expect(thread?.archiveRequest?.status).toBe(
