...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    readonly unavailableInstanceIds?: ReadonlySet<string>;
    ...
    const compactThread = vi.fn(
      (_: ThreadId, _modelSelection?: ModelSelection, _requestId?: MessageId) =>
    ...
    const service: ProviderServiceShape = {
      ...
      getInstanceInfo: (instanceId) => {
        ...
        if (input?.unavailableInstanceIds?.has(raw) === true) {
          return Effect.fail(
            new ProviderUnsupportedError({
              provider: ProviderDriverKind.make(raw),
            }),
          );
        }
    ...
    (scenario) =>
      Effect.gen(function* () {
        ...
        expect(harness.sendTurn.mock.calls.slice(1).map(([request]) => request)).toEqual([
          expect.objectContaining({
            input: withInputOriginNotice("first queued", "voice-transcription"),
            interactionMode: "plan",
          }),
        ...
        expect(
          afterRestore?.messages.find((message) => message.text === "first queued")?.inputOrigin,
  ...
  effectIt.effect("preserves a running turn across accepted and failed steer attempts", () =>
    Effect.gen(function* () {
      ...
      yield* harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-before-failed-steer"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-message-before-failed-steer"),
          role: "user",
          text: "start review",
          attachments: [],
        },
      ...
      yield* harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-set-running-before-failed-steer"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "running",
      ...
      yield* harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-failed-steer"),
        threadId: ThreadId.make("thread-1"),
        message: {
  ...
  it("defers a stopped owner's live continuation-key drift to ProviderService", async () => {
    ...
    await harness.dispatch({
      type: "thread.session.set",
      commandId: CommandId.make("cmd-session-set-stopped-compatible-switch"),
      threadId: ThreadId.make("thread-1"),
      session: {
        threadId: ThreadId.make("thread-1"),
        status: "stopped",
    ...
    await harness.dispatch({
      type: "thread.meta.update",
      commandId: CommandId.make("cmd-meta-stopped-compatible-switch"),
      threadId: ThreadId.make("thread-1"),
      modelSelection: {
        instanceId: ProviderInstanceId.make("codex_work"),
        model: "gpt-5-codex",
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-stopped-compatible-switch"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-stopped-compatible-switch"),
        role: "user",
        text: "continue on work",
        attachments: [],
      },
      modelSelection: {
        instanceId: ProviderInstanceId.make("codex_work"),
  ...
  it("defers a cold switch from an unavailable prior owner to ProviderService", async () => {
    const harness = await createHarness({
    ...
    await harness.dispatch({
      type: "thread.session.set",
      commandId: CommandId.make("cmd-session-set-unavailable-owner"),
      threadId: ThreadId.make("thread-1"),
      session: {
        threadId: ThreadId.make("thread-1"),
        status: "error",
    ...
    await harness.dispatch({
      type: "thread.meta.update",
      commandId: CommandId.make("cmd-meta-unavailable-owner-switch"),
      threadId: ThreadId.make("thread-1"),
      modelSelection: {
        instanceId: ProviderInstanceId.make("codex_work"),
        model: "gpt-5-codex",
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-unavailable-owner-switch"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-unavailable-owner-switch"),
        role: "user",
        text: "continue on the replacement",
        attachments: [],
      },
      modelSelection: {
        instanceId: ProviderInstanceId.make("codex_work"),
  ...
  it("rejects a stopped-thread switch across incompatible continuation groups", async () => {
    ...
    await harness.dispatch({
      type: "thread.session.set",
      commandId: CommandId.make("cmd-session-set-stopped-incompatible-switch"),
      threadId: ThreadId.make("thread-1"),
      session: {
        threadId: ThreadId.make("thread-1"),
        status: "stopped",
    ...
    await harness.dispatch({
      type: "thread.meta.update",
      commandId: CommandId.make("cmd-meta-stopped-incompatible-switch"),
      threadId: ThreadId.make("thread-1"),
      modelSelection: {
        instanceId: ProviderInstanceId.make("codex_work"),
        model: "gpt-5-codex",
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-stopped-incompatible-switch"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-stopped-incompatible-switch"),
        role: "user",
        text: "continue on work",
        attachments: [],
      },
    ...
    await harness.dispatch({
      type: "thread.meta.update",
      commandId: CommandId.make("cmd-meta-restore-stopped-owner"),
      threadId: ThreadId.make("thread-1"),
      modelSelection: {
        instanceId: ProviderInstanceId.make("codex"),
        model: "gpt-5-codex",
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-start-restored-owner"),
      threadId: ThreadId.make("thread-1"),
      message: {
        messageId: asMessageId("user-message-restored-owner"),
        role: "user",
        text: "continue on the original owner",
        attachments: [],
      },
      modelSelection: {
        instanceId: ProviderInstanceId.make("codex"),
  ...
  it("compacts on the failover instance's selection while the desired instance is rate limited", async () => {
    const harness = await createHarness({ failoverInstanceIds: { codex: "codex_work" } });
    const now = "2026-01-01T00:00:00.000Z";
    const threadId = ThreadId.make("thread-1");
    const desired = { instanceId: ProviderInstanceId.make("codex"), model: "gpt-5-codex" };
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-turn-before-failover-compact"),
      threadId,
      message: {
        messageId: asMessageId("user-message-before-failover-compact"),
        role: "user",
        text: "hello",
        attachments: [],
      },
      modelSelection: desired,
      interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
      runtimeMode: "approval-required",
      createdAt: now,
    });
    ...
    await harness.dispatch({
      type: "thread.session.set",
      commandId: CommandId.make("cmd-session-ready-failover-compact"),
      threadId,
      session: {
        threadId,
        status: "ready",
    ...
    await harness.dispatch({
      type: "thread.turn.start",
      commandId: CommandId.make("cmd-failover-compact"),
      threadId,
      message: {
        messageId: asMessageId("user-message-failover-compact"),
        role: "user",
        text: "/compact",
        attachments: [],
      },
    ...
    // The session runs on the sibling; compacting on the rate-limited
    // instance's selection would target a provider that refused the turn.
    expect(harness.compactThread.mock.calls[0]?.[1]).toMatchObject({
      instanceId: ProviderInstanceId.make("codex_work"),
    });
  });
