...
const refreshedRepositoryIdentity = {
  ...
  webUrl: "https://github.com/t3tools/t3code",
};
  ...
  it.effect("refreshes stale persisted identity after successful live resolution", () =>
    Effect.gen(function* () {
      ...
      yield* sql`UPDATE projection_projects
        SET repository_identity_json = json_set(repository_identity_json, '$.webUrl', 'https://github.com/old/repository')
        WHERE project_id = 'project-refresh-identity'`;
      yield* snapshotQuery.getProjectShells();
...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
    ...
    Effect.gen(function* () {
      ...
      yield* sql`
        INSERT INTO projection_thread_sessions (
          ...
          'thread-activity-filter', 'running', 'codex', 'codex', 'full-access',
          'turn-activity-filter', NULL, '2026-02-25T00:00:08.000Z'
        )
      `;
      const counter = makeSqlStatementCounter();
      const context = yield* snapshotQuery
        .getThreadRuntimeContext(ThreadId.make("thread-activity-filter"))
      ...
      if (context._tag === "Some") {
        ...
        assert.equal(context.value.session?.activeTurnId, "turn-activity-filter");
      }
      ...
      yield* sql`
        ...
        WHERE thread_id = 'thread-activity-filter'
      `;
  ...
  it.effect("keeps archived threads out of the main shell snapshot", () =>
    Effect.gen(function* () {
      ...
      // reaches the archived shell snapshot and thread detail.
      yield* sql`
        INSERT INTO projection_thread_pull_requests (
        ...
        VALUES (
          ...
          'pingdotgg/t3code',
          44,
          'https://github.com/pingdotgg/t3code/pull/44',
          'manual',
      ...
      `;
      const archivedShellWithLink = yield* snapshotQuery.getArchivedShellSnapshot();
      const archivedDetailWithLink = yield* snapshotQuery.getThreadDetailById(
        ThreadId.make("thread-archived"),
      );
      ...
      assert.equal(recentArchived.threads[0]?.pullRequests.length, 1);
      assert.deepEqual(
        recentArchived.threads[0]?.pullRequests,
        archivedShellWithLink.threads[0]?.pullRequests,
      );
      assert.deepEqual(
        recentArchived.threads[0]?.pullRequests,
        archivedDetailWithLink._tag === "Some"
          ? archivedDetailWithLink.value.pullRequests
          : undefined,
      );
      ...
      assert.deepEqual(
        ...
        archivedDetailWithLink._tag === "Some"
          ? archivedDetailWithLink.value.linkedPullRequest
          : undefined,
      );
      assert.deepEqual(
      ...
      if (activeContext._tag === "Some") {
        assert.equal(activeContext.value.branch, null);
        assert.equal(activeContext.value.worktreePath, null);
  ...
  it.effect("measures replay payload bytes without decoding event bodies", () =>
    Effect.gen(function* () {
      ...
      // The aggregate filter must count only the target thread's rows — it
      // filters on the persisted stream_id column, so this exercises the real
      // filtered SQL branch rather than the global one.
      yield* sql`
        INSERT INTO orchestration_events (
          event_id, aggregate_kind, stream_id, stream_version, event_type, occurred_at,
          command_id, causation_event_id, correlation_id, actor_kind, payload_json, metadata_json
        ...
        VALUES
          (
            'replay-event-other-1', 'thread', 'thread-other', 1, 'thread.activity-appended',
            '2026-03-01T00:00:04.000Z', NULL, NULL, NULL, 'provider',
            json_object('output', printf('%.*c', 4000, 'x')), '{}'
          )
      `;
      const filtered = yield* snapshotQuery.getEventReplayStats({
        fromSequenceExclusive: 1,
        toSequenceInclusive: 5,
...
projectionSnapshotLayer("ProjectionSnapshotQuery windowed thread detail", (it) => {
  ...
  // An imported thread: history-imported messages carry their ORIGINAL
  // timestamps and a null turn_id, so they sit older than the first real turn's
  // requested_at. Turnless rows are bounded by `created_at >= minAnchorAt`, so
  // unless the terminal page widens its lower bound they belong to no page at
  // all while `hasMore: false` claims history is complete.
  //
  //   imported-1  turn_id NULL  T-02:00
  //   imported-2  turn_id NULL  T-01:00
  //   pre-turn-activity turn_id NULL T-00:30
  //   turn-1      user-msg-1    T00:00
  //   turn-2      user-msg-2    T01:00
  const seedImportedThread = Effect.fnUntraced(function* () {
    const sql = yield* SqlClient.SqlClient;
    ...
    yield* sql`
      ...
      VALUES ('project-i', 'Imported', '/tmp/project-i', '[]',
        '2026-03-01T00:00:00.000Z', '2026-03-01T00:00:00.000Z', NULL)
    `;
    yield* sql`
      INSERT INTO projection_threads (
      ...
      VALUES ('thread-i', 'project-i', 'Imported thread',
        '{"provider":"codex","model":"gpt-5-codex"}', 'full-access', 'default',
        'turn-2', 0, 0, 0, '2026-02-28T22:00:00.000Z', '2026-03-01T01:00:00.000Z', NULL)
    `;

    for (const { id, at } of [
      { id: "imported-1", at: "2026-02-28T22:00:00.000Z" },
      { id: "imported-2", at: "2026-02-28T23:00:00.000Z" },
    ]) {
      yield* sql`
        INSERT INTO projection_thread_messages (
          message_id, thread_id, turn_id, role, text, is_streaming, created_at, updated_at
        )
        VALUES (${id}, 'thread-i', NULL, 'user', ${"imported " + id}, 0, ${at}, ${at})
      `;
    }
    yield* sql`
      INSERT INTO projection_thread_activities (
      ...
      VALUES ('pre-turn-activity', 'thread-i', NULL, 'info', 'context-window.updated',
        'usage', '{"usedTokens":1}', '2026-02-28T23:30:00.000Z')
    `;

    for (const { turn, pendingMessage, at } of [
      { turn: "turn-1", pendingMessage: "user-msg-1", at: "2026-03-01T00:00:00.000Z" },
      { turn: "turn-2", pendingMessage: "user-msg-2", at: "2026-03-01T01:00:00.000Z" },
    ]) {
      yield* sql`
        INSERT INTO projection_turns (
          thread_id, turn_id, pending_message_id, state, requested_at, started_at, completed_at,
        ...
        VALUES ('thread-i', ${turn}, ${pendingMessage}, 'completed', ${at}, ${at}, ${at}, '[]')
      `;
      yield* sql`
        INSERT INTO projection_thread_messages (
          message_id, thread_id, turn_id, role, text, is_streaming, created_at, updated_at
        )
        VALUES (${pendingMessage}, 'thread-i', NULL, 'user', ${"prompt for " + turn}, 0, ${at}, ${at})
      `;
      yield* sql`
        INSERT INTO projection_thread_messages (
          message_id, thread_id, turn_id, role, text, is_streaming, created_at, updated_at
        )
        VALUES (${turn + "-reply"}, 'thread-i', ${turn}, 'assistant', ${"reply from " + turn}, 0, ${at}, ${at})
      `;
    }
    ...
    for (const projector of Object.values(ORCHESTRATION_PROJECTOR_NAMES)) {
      yield* sql`
        INSERT INTO projection_state (projector, last_applied_sequence, updated_at)
        VALUES (${projector}, 7, '2026-03-01T01:00:00.000Z')
      `;
    }
  });

  const threadI = ThreadId.make("thread-i");

  it.effect("the terminal page sweeps up turnless rows older than the first turn", () =>
    Effect.gen(function* () {
      yield* seedImportedThread();
      const snapshotQuery = yield* ProjectionSnapshotQuery;

      // Page 1: newest user turn only. Imported rows stay out, and history is
      // correctly reported as incomplete.
      const firstPage = yield* snapshotQuery.getThreadDetailSnapshot(threadI, { turnLimit: 1 });
      assert.equal(firstPage._tag, "Some");
      if (firstPage._tag !== "Some") return;
      assert.deepEqual(messageIds(firstPage.value), ["turn-2-reply", "user-msg-2"]);
      assert.equal(firstPage.value.page?.hasMore, true);
      const cursor = firstPage.value.page?.beforeCursor;
      assert.notEqual(cursor, null);
      if (cursor === null || cursor === undefined) return;

      // Page 2 is terminal (no turn older than turn-1). It must therefore
      // include every remaining turnless row — the imported messages and the
      // pre-first-turn activity — before declaring history complete.
      const olderPage = yield* snapshotQuery.getThreadDetailSnapshot(threadI, {
        turnLimit: 1,
        beforeCursor: cursor,
      });
      ...
      assert.deepEqual(activityIds(olderPage.value), ["pre-turn-activity"]);
      assert.equal(olderPage.value.page?.hasMore, false);
      assert.equal(olderPage.value.page?.beforeCursor, null);

      // Nothing is lost across the two pages: their union is the whole thread.
      const full = yield* snapshotQuery.getThreadDetailSnapshot(threadI);
      assert.equal(full._tag, "Some");
      if (full._tag !== "Some") return;
      assert.deepEqual(
        [...messageIds(firstPage.value), ...messageIds(olderPage.value)].toSorted(),
        messageIds(full.value),
      );
    }),
  );

  it.effect("a single-page window of an imported thread still includes imported rows", () =>
    Effect.gen(function* () {
      // The first page is also the terminal page here, so the widened lower
      // bound must apply on page one too.
      yield* seedImportedThread();
      const snapshotQuery = yield* ProjectionSnapshotQuery;

      const snapshot = yield* snapshotQuery.getThreadDetailSnapshot(threadI, { turnLimit: 5 });
      assert.equal(snapshot._tag, "Some");
      ...
      assert.deepEqual(activityIds(snapshot.value), ["pre-turn-activity"]);
      assert.equal(snapshot.value.page?.hasMore, false);
    }),
  );
