...
const make = Effect.gen(function* () {
      ...
      yield* orchestrationEngine
        .dispatch({
          ...
          message: {
            ...
            ...(turnStart.value.message.inputOrigin !== undefined
              ? { inputOrigin: turnStart.value.message.inputOrigin }
              : {}),
          },
        })
        .pipe(
  ...
  const setThreadSessionErrorOnTurnStartFailure = Effect.fnUntraced(function* (input: {
    ...
    // A failed steer must not settle the original turn. Genuine startup
    // failures use upstream's error state so the thread can retry cleanly.
    ...
    yield* setThreadSession({
      ...
      session: {
        ...
        status:
          session?.status === "stopped" ? "stopped" : preserveActiveTurn ? "running" : "error",
      ...
      Effect.catchTag("OrchestrationCommandInvariantError", (error) => {
        const expectedSessionMismatch =
          error.commandType === "thread.session.set" &&
          error.detail.includes(" but the write expected ") &&
          error.detail.endsWith("; dropping stale session set");
        return expectedSessionMismatch
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    if (isCompactCommand) {
      ...
      yield* Effect.gen(function* () {
        ...
        // instance; compact on the selection that session actually runs.
        yield* providerService.compactThread(
          event.payload.threadId,
          event.payload.modelSelection !== undefined
            ? ensuredSession.effectiveModelSelection
            : undefined,
  ...
  const processForkRequested = Effect.fn("processForkRequested")(function* (
    ...
    const destination = yield* resolveThreadShell(event.payload.threadId);
  ...
  const processDomainEvent = Effect.fn("processDomainEvent")(function* (
    ...
    switch (event.type) {
      ...
      case "thread.settled": {
        ...
        // Background work keeps the provider session alive after settlement.
        if (
