  ...
  function* () {
    ...
    const applyThreadsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.turn-diff-completed": {
          ...
          const preserveNewerLatestTurn =
            Option.isSome(latestTurn) &&
            latestTurn.value.turnId !== event.payload.turnId &&
            (latestTurn.value.checkpointTurnCount === null ||
              latestTurn.value.checkpointTurnCount > event.payload.checkpointTurnCount);
          yield* projectionThreadRepository.upsert({
            ...existingRow.value,
            latestTurnId: preserveNewerLatestTurn
    ...
    const applyThreadMessagesProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.created":
          ...
          // The fork's per-message artifacts have no cascading foreign key,
          // and a superseded thread.deleted deliberately skips artifact
          // cleanup on replay — reset them with the messages they annotate.
          yield* sql`
            DELETE FROM projection_message_speech
            WHERE thread_id = ${event.payload.threadId}
          `.pipe(
            Effect.mapError(
              toPersistenceSqlError("ProjectionPipeline.applyThreadMessagesProjection:reset"),
            ),
          );
          yield* sql`
            DELETE FROM projection_message_summary
    ...
    const applyThreadTurnsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.fork-requested":
          yield* projectionTurnRepository.copyCompletedTurnsForFork({
            sourceThreadId: event.payload.sourceThreadId,
            destinationThreadId: event.payload.threadId,
          });
          return;
        ...
        case "thread.session-set": {
          ...
          // thread. Treat the old turn as interrupted: steering can open the
          // new turn without a final assistant response from the old one.
        ...
        case "thread.message-sent": {
          ...
          const turnStillRunning =
            (Option.isSome(existingTurn) && existingTurn.value.state === "running") ||
          ...
          if (Option.isSome(existingTurn)) {
            ...
            yield* projectionTurnRepository.upsertByTurnId({
              ...
              state: turnStillRunning
                ? existingTurn.value.state
                : (preservedTerminalState ?? nextState),
              checkpointTurnCount: event.payload.checkpointTurnCount,
    ...
    const applyAttachmentSideEffects = Effect.fn("applyAttachmentSideEffects")(
      ...
      // The fork's speech/summary pruning issues SQL from the deferred cleanup,
      // which runs after the outer transaction commits, so it gets the plain
      // (non-transactional) client explicitly.
