...
> =>
  Effect.gen(function* () {
            ...
            promptStart(payload).pipe(
              Effect.flatMap((handle) =>
                handle.start.pipe(
                  ...
                  Effect.andThen(restore(handle.awaitResult)),
                ),
              ),
            ),
          )
        : serializedPrompt(payload, promptOptions);

    const serializedCancel = Effect.gen(function* () {
      const started = yield* getStartedState;
      ...
      if (options.cancelBehavior !== "wait-for-prompt") {
        ...
        // Await the notification write so a replacement prompt cannot race it on the wire.
    ...
    const concurrentCancel = getStartedState.pipe(
      Effect.flatMap((started) =>
        Effect.uninterruptible(
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              yield* Effect.forEach(registration.fibers, Fiber.interrupt, { discard: true });
              yield* acp.agent.cancel({ sessionId: started.sessionId }).pipe(Effect.ignore);
    ...
    return {
      ...
      cancel:
        options.promptConcurrency === "concurrent"
          ? concurrentCancel
          : options.cancelBehavior === "wait-for-prompt"
            ? promptDispatchSemaphore.withPermit(serializedCancel)
            : serializedCancel,
