...
function makeHarness(config?: {
  ...
  return {
    layer: Layer.effect(
      ...
      Layer.provide(
        config?.spawner
          ? Layer.succeed(ChildProcessSpawner.ChildProcessSpawner, config.spawner)
          : Layer.empty,
      ),
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("forks a persisted Claude session without starting a live query", () => {
    ...
    const forkSession: NonNullable<ClaudeAdapterLiveOptions["forkSession"]> = async (...args) => {
      forkCalls.push(args);
  ...
  it.effect("preserves unknown fork boundaries and rejects invalid fork output", () => {
    ...
    return Effect.gen(function* () {
      ...
      const result = yield* adapter.forkSession!(input);
      assert.deepEqual(result.resumeCursor, {
        threadId: "destination-thread",
  ...
  for (const scenario of [
    ...
    "reorders retained messages",
  ] as const) {
    it.effect(`validates full-session Claude forks when the fork ${scenario}`, () => {
      const history = [
        claudeHistoryMessage({ type: "system", uuid: "system-init" }),
        claudeHistoryMessage({ type: "user", uuid: "user-1", content: "first" }),
        claudeHistoryMessage({ type: "assistant", uuid: "assistant-1", content: "reply one" }),
        claudeHistoryMessage({ type: "user", uuid: "user-2", content: "second" }),
        claudeHistoryMessage({ type: "assistant", uuid: "assistant-2", content: "reply two" }),
      ];
      const forkHistory = history.slice(1).map((message) => ({
        ...message,
      ...
      if (scenario === "omits system message bodies") {
        ...
        forkHistory.splice(
          ...
          claudeHistoryMessage({ type: "system", uuid: "fork-init" }),
        );
      } else if (scenario === "restores a compact conversation prefix") {
        forkHistory.unshift(
          claudeHistoryMessage({ type: "user", uuid: "restored-user", content: "earlier turn" }),
          claudeHistoryMessage({
            type: "assistant",
      ...
      } else if (scenario === "drops a retained message") {
        forkHistory.splice(1, 1);
      } else if (scenario === "changes a retained message") {
        ...
        [forkHistory[0], forkHistory[2]] = [forkHistory[2]!, forkHistory[0]!];
      }
      const harness = makeHarness({
        forkSession: async () => ({ sessionId: CLAUDE_FORK_SESSION_ID }),
        getSessionMessages: async (sessionId) =>
          sessionId === CLAUDE_ORIGINAL_SESSION_ID ? history : forkHistory,
      });
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        const fork = adapter.forkSession!({
          sourceThreadId: THREAD_ID,
          destinationThreadId: RESUME_THREAD_ID,
          sourceResumeCursor: {
        ...
        if (
          ...
          assert.deepEqual((yield* fork).resumeCursor, {
            ...
            turnCount: 2,
            turnStartMessageIds: ["fork-user-1", "fork-user-2"],
          });
        } else {
  ...
  it.effect("warns for unmapped Claude limits and names the probed model bucket", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* drainSdkMessages;
      const unmappedUpdates = runtimeEvents.filter(
        (event) => event.type === "account.rate-limits.updated",
      );
      assert.equal(unmappedUpdates.length, 2);
      assert.isTrue(
        unmappedUpdates.every(
          (event) =>
            event.type === "account.rate-limits.updated" &&
            event.payload.limits === undefined &&
