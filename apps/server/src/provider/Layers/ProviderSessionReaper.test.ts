...
describe("ProviderSessionReaper", () => {
  ...
  it("reaps background work after the maximum extension expires", async () => {
    const threadId = ThreadId.make("thread-reaper-background-work-expired");
    const lastSeenAt = DateTime.formatIso(
      DateTime.subtractDuration(await Effect.runPromise(DateTime.now), 11_000),
    );
    const harness = await createHarness({
    ...
    await runtime!.runPromise(
      repository.upsert({
        ...
        status: "running",
        lastSeenAt,
        resumeCursor: { opaque: "resume-background-work-expired" },
        runtimePayload: null,
      }),
    );
