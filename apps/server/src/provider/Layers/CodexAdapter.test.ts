...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("stamps turn abort events with the session generation", () =>
    Effect.gen(function* () {
      const { adapter, runtime, session } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      ...
      NodeAssert.ok(session.sessionGenerationId);
      yield* runtime.emit({
        id: asEventId("evt-turn-aborted-generation"),
        kind: "notification",
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-1"),
        turnId: asTurnId("turn-1"),
      ...
      const firstEvent = yield* Fiber.join(firstEventFiber);
      NodeAssert.equal(firstEvent._tag, "Some");
      if (firstEvent._tag !== "Some" || firstEvent.value.type !== "turn.aborted") {
        return;
      }
      NodeAssert.equal(firstEvent.value.payload.sessionGenerationId, session.sessionGenerationId);
    }),
...
usageLimitLayer("CodexAdapterLive usage limits", (it) => {
  ...
  it.effect("reads a rate-limit snapshot seen earlier in the session", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startUsageLimitRuntime();
      const eventsFiber = yield* adapter.streamEvents.pipe(
        // The sparse update still flows as a raw-only rate-limits event on the
        // fork (failover reads the raw half), so one more event precedes the stop.
        Stream.take(4),
        Stream.runCollect,
        Effect.forkChild,
      );
