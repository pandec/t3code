    ...
    return [
      {
        ...
        ...runtimeEventBase(event, canonicalThreadId),
        // Raw notification rides beside the typed windows for the fork's
        // opaque usage snapshot and gateway-pool attribution.
        payload: { ...(limits ? { limits } : {}), rateLimits: event.payload ?? {} },
      },
    ];
  }
          ...
          ...(mcpSession
            ? {
                ...
                appServerArgs: [
                  ...
                  "-c",
                  `mcp_servers.t3-code.tool_timeout_sec=${McpProviderSession.MCP_TOOL_CALL_TIMEOUT_MS / 1000}`,
                ],
            ...
            const mappedEvents = mapToRuntimeEvents(event, event.threadId).map((runtimeEvent) => {
              const eventWithTokenUsage =
                runtimeEvent.type === "turn.completed" && runtimeEvent.turnId
                  ...
                  : runtimeEvent.type === "turn.aborted" && runtimeEvent.turnId
                    ? ({
                        ...runtimeEvent,
                        payload: {
                          ...runtimeEvent.payload,
                    ...
                    : runtimeEvent;
              return stampSessionGeneration(eventWithTokenUsage, sessionGenerationId);
            });
            // The synthetic error carries the same generation stamp as the
            // mapped events, or a replaced session's late limit would land
            // on its successor.
            const runtimeEvents = usageLimitError
              ? [stampSessionGeneration(usageLimitError, sessionGenerationId), ...mappedEvents]
              : mappedEvents;
