...
it.effect("ProviderServiceLive shutdown leaves settled session rows untouched", () =>
  Effect.gen(function* () {
    ...
    // Startup backfills continuation metadata; shutdown must leave this row intact.
...
routing.layer("ProviderServiceLive routing", (it) => {
  it.effect("forks into a distinct stopped binding without mutating the source", () =>
    Effect.gen(function* () {
      ...
      yield* provider.startSession(sourceThreadId, {
        ...
        cwd: fixtureCwd("fork-source-project"),
      ...
      assert.equal(
        (destination.runtimePayload as { readonly continuationKey?: unknown } | null)
          ?.continuationKey,
    ...
    Effect.gen(function* () {
      ...
      yield* provider.sendTurn({
        ...
        attachments: [
          {
            type: "image",
            id: "thread-window-accessibility-12345678-1234-1234-1234-123456789abc",
            name: "editor.png",
            mimeType: "image/png",
            sizeBytes: 123,
            source: {
      ...
      Effect.gen(function* () {
        ...
        yield* provider.sendTurn({
          ...
          attachments: [
            {
              type: "image",
              id: "thread-window-accessibility-compaction-12345678-1234-1234-1234-123456789abc",
              name: "terminal.png",
              mimeType: "image/png",
              sizeBytes: 123,
              source: {
    ...
    Effect.gen(function* () {
      ...
      yield* provider.sendTurn({
        ...
        attachments: Array.from({ length: 8 }, (_, index) => ({
          type: "image" as const,
          id: `thread-window-text-limit-${index}2345678-1234-1234-1234-123456789abc`,
      ...
      for (let index = 0; index < 8; index += 1) {
        assert.include(
          turnInput.input ?? "",
          `thread-window-text-limit-${index}2345678-1234-1234-1234-123456789abc.png`,
        );
      }
    }),
  );
  ...
  it.effect("falls back to the requested directory when a persisted cwd no longer exists", () =>
    Effect.gen(function* () {
      ...
      routing.codex.startSession.mockClear();
      const requestedCwd = fixtureCwd("project-root-after-removed-worktree");
      ...
      assert.equal(routing.codex.startSession.mock.calls[0]?.[0]?.cwd, requestedCwd);
    }),
  );
  ...
  it.effect("keeps an imported worktree cwd authoritative across provider restarts", () =>
    Effect.gen(function* () {
      ...
      const importedCwd = fixtureCwd("imported-worktree");
      yield* directory.upsert({
        threadId,
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
      ...
      assert.equal(startInput.cwd, importedCwd);
      ...
      assert.equal(routing.codex.startSession.mock.calls[0]?.[0]?.cwd, importedCwd);
    }),
  );
  ...
  it.effect("does not persist running after a concurrent send is interrupted", () =>
    Effect.gen(function* () {
      ...
      // Fork semantics: every thread-scoped method runs under the per-thread
      // lock, so provider.interruptTurn cannot overtake an in-flight sendTurn
      // (upstream's original interleaving deadlocks here by design). The
      // hazard this test pins — an interrupted send persisting "running" —
      // still exists for external fiber interruption, so interrupt the send
      // fiber directly instead.
  ...
  it.effect("reuses persisted cursor and cwd across compatible instances after a restart", () =>
    Effect.gen(function* () {
      ...
      const sharedCwd = fixtureCwd("shared-claude-project");
      yield* Effect.gen(function* () {
        const repository = yield* ProviderSessionRuntime.ProviderSessionRuntimeRepository;
  ...
  it.effect("uses persisted continuation keys to resume threads whose owner was removed", () =>
    Effect.gen(function* () {
      ...
      const persistedCwd = fixtureCwd("removed-owner-project");
        ...
        for (const binding of [
          {
            ...
            providerInstanceId: ProviderInstanceId.make("claude_removed_matching"),
...
fanout.layer("ProviderServiceLive fanout", (it) => {
  it.effect("persists a runtime cwd change onto the binding", () =>
    Effect.gen(function* () {
      ...
      const workspaceCwd = fixtureCwd("project-cwd-binding");
      const worktreeCwd = NodePath.join(workspaceCwd, ".claude/worktrees/feature");
      ...
      assert.equal(runtimePayload?.cwd, worktreeCwd);
  ...
  for (const [driver, adapter] of [
    ...
    it.effect(`expands quotes and bound comments as JSON data for ${driver}`, () =>
      Effect.gen(function* () {
        ...
        // The fork validates attachment ownership, so the fixture id must
        // embed the target thread's attachment segment.
        const attachment = {
          type: "file" as const,
...
const turnAnalytics = makeProviderServiceLayer({
  ...
  registry: makeInstanceRegistry([
    {
      instanceId: codexInstanceId,
      driverKind: CODEX_DRIVER,
      continuationKey: "codex:shared-analytics-test-home",
      adapter: primaryAnalyticsCodex.adapter,
    },
    {
      instanceId: secondaryCodexInstanceId,
      driverKind: CODEX_DRIVER,
      continuationKey: "codex:shared-analytics-test-home",
      adapter: secondaryAnalyticsCodex.adapter,
    },
  ]),
});
...
turnAnalytics.layer("ProviderServiceLive turn analytics", (it) => {
  ...
  it.effect("keeps request metadata with out-of-order responses across threads", () =>
    Effect.gen(function* () {
      recordedTurnAnalytics.reset();
      const provider = yield* ProviderService.ProviderService;
      const firstThreadId = asThreadId("thread-turn-analytics-overlap-first");
      ...
      for (const [turnId, suffix, threadId] of [
        [firstTurnId, "first", firstThreadId],
        [secondTurnId, "second", secondThreadId],
      ] as const) {
        primaryAnalyticsCodex.emit({
          type: "turn.started",
  ...
  it.effect("defers terminal analytics until each cross-thread request is associated", () =>
    Effect.gen(function* () {
      recordedTurnAnalytics.reset();
      const provider = yield* ProviderService.ProviderService;
      const firstThreadId = asThreadId("thread-turn-analytics-overlap-fast-first");
  ...
  it.effect("bounds deferred completions and drains them after a send is canceled", () =>
    Effect.gen(function* () {
      recordedTurnAnalytics.reset();
      const provider = yield* ProviderService.ProviderService;
      ...
      primaryAnalyticsCodex.sendTurn.mockImplementationOnce((input) =>
        Effect.gen(function* () {
          yield* Deferred.succeed(sendStarted, undefined);
          yield* Deferred.await(sendRelease);
          return { threadId: input.threadId, turnId: turnIds.at(-1)! };
        }),
      );
      ...
      const send = yield* provider
        .sendTurn({
          threadId,
          input: "bound deferred analytics",
          attachments: [],
          interactionMode: "default",
        })
        .pipe(Effect.forkChild);
  ...
  it.effect("clears stale instance analytics when its replaced session exits", () =>
    Effect.gen(function* () {
      recordedTurnAnalytics.reset();
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("thread-turn-analytics-stale-exit");
      const initialSession = yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      const turn = yield* provider.sendTurn({
        threadId,
        input: "measure the replaced generation",
        attachments: [],
        modelSelection: createModelSelection(codexInstanceId, "stale-model"),
      });
      yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
      ...
      const exitProcessed = yield* provider.streamEvents.pipe(
        Stream.filter(
          (event) =>
            event.type === "thread.state.changed" &&
            event.eventId === asEventId("evt-turn-analytics-stale-exit-marker"),
        ),
        Stream.runHead,
        Effect.forkChild,
      );
      ...
      primaryAnalyticsCodex.emit({
        type: "session.exited",
        eventId: asEventId("evt-turn-analytics-stale-exit"),
        provider: CODEX_DRIVER,
        createdAt: "2026-01-01T00:00:00.000Z",
        threadId,
        payload: {
      ...
      primaryAnalyticsCodex.emit({
        type: "thread.state.changed",
        eventId: asEventId("evt-turn-analytics-stale-exit-marker"),
        provider: CODEX_DRIVER,
        createdAt: "2026-01-01T00:00:01.000Z",
        threadId,
        payload: { state: "compacted" },
      });
      assert.equal((yield* Fiber.join(exitProcessed))._tag, "Some");
      yield* advanceTestClock(50);

      const staleCompletion = yield* provider.streamEvents.pipe(
        Stream.filter(
          (event) =>
            event.type === "turn.completed" &&
            event.eventId === asEventId("evt-turn-analytics-after-stale-exit"),
        ),
        Stream.runHead,
        Effect.forkChild,
      );
      ...
      primaryAnalyticsCodex.emit({
        type: "turn.completed",
        eventId: asEventId("evt-turn-analytics-after-stale-exit"),
        provider: CODEX_DRIVER,
        createdAt: "2026-01-01T00:00:01.000Z",
        threadId,
        turnId: turn.turnId,
      ...
      assert.equal((yield* Fiber.join(staleCompletion))._tag, "Some");
  ...
  it.effect("ignores a stale-generation abort without clearing current turn analytics", () =>
    Effect.gen(function* () {
      recordedTurnAnalytics.reset();
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("thread-turn-analytics-stale-abort");
      const initialSession = yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      const turn = yield* provider.sendTurn({
        threadId,
        input: "measure only the replacement generation",
        attachments: [],
        interactionMode: "plan",
        modelSelection: createModelSelection(codexInstanceId, "current-model"),
      });

      primaryAnalyticsCodex.emit({
        type: "turn.aborted",
        eventId: asEventId("evt-turn-analytics-stale-abort"),
        provider: CODEX_DRIVER,
        createdAt: "2026-01-01T00:00:00.000Z",
        threadId,
        turnId: turn.turnId,
        payload: {
          reason: "Interrupted by replaced session",
          sessionGenerationId: initialSession.sessionGenerationId,
        },
      });
      yield* advanceTestClock(50);
      assert.notEqual(currentSession.sessionGenerationId, initialSession.sessionGenerationId);
      assert.equal(recordedTurnAnalytics.eventsByName("provider.turn.completed").length, 0);

      const currentCompletion = yield* provider.streamEvents.pipe(
        Stream.filter(
          (event) =>
            event.type === "turn.completed" &&
            event.eventId === asEventId("evt-turn-analytics-current-completion"),
        ),
        Stream.runHead,
        Effect.forkChild,
      );
      ...
      primaryAnalyticsCodex.emit({
        type: "turn.completed",
        eventId: asEventId("evt-turn-analytics-current-completion"),
        provider: CODEX_DRIVER,
        createdAt: "2026-01-01T00:00:01.000Z",
        threadId,
        turnId: turn.turnId,
        payload: {
          state: "completed",
          sessionGenerationId: currentSession.sessionGenerationId,
        },
      });
      assert.equal((yield* Fiber.join(currentCompletion))._tag, "Some");

      const completed = recordedTurnAnalytics.eventsByName("provider.turn.completed");
      assert.equal(completed.length, 1);
      assert.equal(completed[0]?.properties?.terminalStatus, "completed");
      assert.equal(completed[0]?.properties?.model, "current-model");
      assert.equal(completed[0]?.properties?.interactionMode, "plan");
    }),
  );
...
describe("agent browser access", () => {
  ...
  const startSessionWith = (
    ...
    Effect.gen(function* () {
      ...
      const providerLayer = makeProviderServiceLive({
        ...
        Layer.provide(
          Layer.mock(AgentVoiceReply)({ available: Effect.succeed(options?.available ?? false) }),
        ),
        Layer.provide(
          ServerSettings.ServerSettingsService.layerTest({
            ...
            voice: {
              enableAgentVoiceReplies:
                options?.available !== undefined && options.enabled !== false,
            },
          }),
        ),
        Layer.provide(serverConfigTestLayer),
  ...
  it.effect("gates voice MCP credentials on speech availability and the user toggle", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(
        ...
        [{ threadId: enabledThread, capabilities: ["pull-requests", "voice"] }],
      );
      const unavailableThread = asThreadId("thread-voice-unavailable");
      assert.deepEqual(
        yield* startSessionWith(false, unavailableThread, undefined, { available: false }),
        [{ threadId: unavailableThread, capabilities: ["pull-requests"] }],
      );
      const disabledThread = asThreadId("thread-voice-disabled");
      assert.deepEqual(
        yield* startSessionWith(false, disabledThread, undefined, {
          available: true,
          enabled: false,
        }),
  ...
  it.effect("keeps voice access global while browser and device access resolve per project", () =>
    Effect.gen(function* () {
      const threadId = asThreadId("thread-project-device-and-global-voice");
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["device", "pull-requests", "voice"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );

  // The capability set on the credential is the observable that matters: a
  // session always gets a credential (the pull request toolkit is never
  // withheld), and preview, device, and voice capabilities grant or deny those tools.
