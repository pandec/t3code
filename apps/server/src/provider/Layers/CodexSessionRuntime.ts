...
export const CodexResumeCursorSchema = Schema.Struct({
  ...
  // Set for imported external sessions: a failed resume must surface as an
  // error instead of silently falling back to a fresh native thread.
...
export interface CodexSessionRuntimeShape {
  ...
  readonly readAccountUsage: Effect.Effect<unknown, CodexSessionRuntimeError>;
...
function readResumeCursorStrictResume(
  resumeCursor: ProviderSession["resumeCursor"],
): boolean | undefined {
...
interface CodexThreadOpenClient {
  ...
  readonly request: <M extends Exclude<CodexThreadOpenMethod, "thread/resume">>(
    method: M,
...
export const openCodexThread = (input: {
  ...
  readonly strictResume?: boolean | undefined;
  readonly forkThreadId?: string | undefined;
}): Effect.Effect<
  CodexThreadOpenResponse | typeof CodexThreadResumeMetadata.Type,
  CodexErrors.CodexAppServerError
> => {
  ...
  const resumeMetadata = (
    ...
    input.client.raw
      .request("thread/resume", { ...params, excludeTurns: true })
      .pipe(
  ...
  if (input.strictResume === true) {
    ...
    return resumeMetadata({ threadId: resumeThreadId, ...strictParams });
  }
  return resumeMetadata({ threadId: resumeThreadId, ...startParams }).pipe(
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      readAccountUsage: initialize.pipe(
        Effect.andThen(client.request("account/rateLimits/read", null)),
      ),
