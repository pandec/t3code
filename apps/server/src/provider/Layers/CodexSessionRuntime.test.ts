...
describe("openCodexThread", () => {
  const rawResumeClient = (
    onResume: (
      payload: CodexRpc.ClientRequestParamsByMethod["thread/resume"] & {
        readonly excludeTurns?: boolean;
      },
    ) => Effect.Effect<unknown, CodexErrors.CodexAppServerError>,
    onStart: <M extends "thread/start" | "thread/fork">(
      method: M,
      payload: CodexRpc.ClientRequestParamsByMethod[M],
    ) => Effect.Effect<CodexRpc.ClientRequestResponsesByMethod[M], CodexErrors.CodexAppServerError>,
  ) => ({
    raw: {
      request: (_method: "thread/resume", payload: Parameters<typeof onResume>[0]) =>
        onResume(payload),
    },
    request: onStart,
  });
  ...
  it.effect("forks the persisted Codex thread when requested", () =>
    Effect.gen(function* () {
      ...
      const client = rawResumeClient(
        () => Effect.die("Fork must not resume"),
        <M extends "thread/start" | "thread/fork">(
          method: M,
          payload: CodexRpc.ClientRequestParamsByMethod[M],
        ) => {
      ...
      for (const invalidMetadata of [
        ...
        const error = yield* openCodexThread({
          client: rawResumeClient(
            ...
            () => Effect.die("Invalid resume metadata must not start a fresh thread"),
          ),
          threadId: ThreadId.make("thread-1"),
          runtimeMode: "full-access",
          cwd: "/tmp/project",
  ...
  it.effect("does not fall back to thread/start when strictResume is set", () =>
    Effect.gen(function* () {
      ...
      const client = rawResumeClient(
        ...
        (method) => {
          calls.push(method);
          return Effect.die("strict resume must not start a fresh thread");
        },
      );
      ...
      NodeAssert.equal((payloads[0] as Record<string, unknown>).excludeTurns, true);
    }),
  );
