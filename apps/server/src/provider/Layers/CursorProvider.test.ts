  ...
  it("discovers recursive project skills with project precedence", async () =>
    await runNode(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        // macOS returns a symlinked tmpdir (/var -> /private/var); discovery
        // reports resolved paths, so resolve the roots up front too.
  ...
  it("treats a symlinked skill outside the root as a package boundary", async () =>
    await runNode(
      Effect.gen(function* () {
        ...
        // Discovery reports real paths; macOS temp directories sit behind a
