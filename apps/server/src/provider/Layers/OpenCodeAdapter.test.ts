...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapterLive", (it) => {
  ...
  it.effect("keeps an idle event from completing a turn while its abort request is pending", () =>
    Effect.gen(function* () {
      ...
      const startedSession = yield* adapter.startSession({
        provider: ProviderDriverKind.make("opencode"),
        threadId,
        runtimeMode: "full-access",
      });
      ...
      NodeAssert.equal(
        aborted?.type === "turn.aborted" ? aborted.payload.sessionGenerationId : undefined,
        startedSession.sessionGenerationId,
      );
      const sessions = yield* adapter.listSessions();
      const session = sessions.find((candidate) => candidate.threadId === threadId);
      NodeAssert.equal(session?.status, "ready");
