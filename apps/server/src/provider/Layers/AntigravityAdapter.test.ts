...
it.layer(layer)("AntigravityAdapter", (it) => {
  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      // macOS hands back symlinked temp roots; the adapter realpath's the
      // parent of every request, so a symlinked cwd would read as an escape.
      const cwd = yield* fs.realPath(yield* fs.makeTempDirectoryScoped({ prefix: "t3-agy-fs-" }));
      const outside = yield* fs.realPath(
        yield* fs.makeTempDirectoryScoped({ prefix: "t3-agy-outside-" }),
      );
