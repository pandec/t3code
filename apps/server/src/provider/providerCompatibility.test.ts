...
describe("provider compatibility", () => {
  it("bundles a compatibility policy for every upstream built-in harness", () => {
    for (const builtIn of BUILT_IN_DRIVERS) {
      // Hermes is fork-only; no compatibility range has been established for it.
      if (builtIn.driverKind === "hermes") continue;
      assert.isDefined(
        resolveProviderCompatibility(
