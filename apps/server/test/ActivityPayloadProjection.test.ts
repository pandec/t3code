  ...
  it("keeps current web and mobile derived output identical for every tool item type", () => {
    for (const activity of fixtures) {
      ...
      if (activity === fixtures[4]) {
        ...
        expect(entry?.toolData).toEqual(expectedToolData);
        ...
        if (mobileGroup?.type === "activity-group") {
          const mobileDetail = mobileGroup.activities[0]?.getFullDetail();
          expect(mobileDetail).toContain('"server": "repository"');
          expect(mobileDetail).toContain('"tool": "search"');
          expect(mobileDetail).not.toContain("mcp bulk is dropped");
        }
        continue;
      }
...
describe("thread message projection", () => {
  ...
  const activities = [
    makeToolLifecycleActivity("window-update", "tool.updated", { toolCallId: "window-call" }),
    makeToolLifecycleActivity("window-complete", "tool.completed", { toolCallId: "window-call" }),
  ];
  const snapshot = {
    snapshotSequence: 17,
