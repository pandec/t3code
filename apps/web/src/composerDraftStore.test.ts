...
describe("composerDraftStore input origin", () => {
  const draftId = DraftId.make("draft-input-origin");

  beforeEach(resetComposerDraftStore);
  afterEach(resetComposerDraftStore);

  it("records voice transcription provenance", () => {
    useComposerDraftStore.getState().setPrompt(draftId, "Read this back", "voice-transcription");

    expect(draftByKey(draftId)?.inputOrigin).toBe("voice-transcription");
  });

  it("retains provenance across prompt updates", () => {
    const store = useComposerDraftStore.getState();
    store.setPrompt(draftId, "Read this back", "voice-transcription");
    store.setPrompt(draftId, "Read this back with more detail");

    expect(draftByKey(draftId)?.inputOrigin).toBe("voice-transcription");
  });

  it("persists and hydrates provenance", () => {
    useComposerDraftStore.getState().setPrompt(draftId, "Read this back", "voice-transcription");
  ...
  it("removes provenance whenever the prompt is cleared", () => {
    const store = useComposerDraftStore.getState();
    store.setModelSelection(draftId, modelSelection(CODEX_DRIVER, "gpt-5.1"));
    store.setPrompt(draftId, "Read this back", "voice-transcription");

    store.setPrompt(draftId, "");
    expect(draftByKey(draftId)?.inputOrigin).toBeUndefined();
...
describe("persistComposerDraftContentNow", () => {
  ...
  it("acknowledges transferred context records only once they are in the readback", () => {
    const context = makeTerminalContext({ id: "queued-context" });
    const store = useComposerDraftStore.getState();
    store.setPrompt(threadRef, "queued text");
    store.addTerminalContexts(threadRef, [context], { appendReference: false });
    const prompt = draftFor(threadId, TEST_ENVIRONMENT_ID)?.prompt ?? "";

    expect(persistComposerDraftContentNow(threadRef, { prompt, attachments: [] })).toBe(true);

    // A record that never reaches storage must not be acknowledged: the queued
    // row is the only durable copy of its payload until the readback proves it.
    // Swapping the persist storage out drops the write on the floor; the
    // readback still reads the previous flush, which lacks the new record.
    const options = useComposerDraftStore.persist.getOptions();
    const originalStorage = options.storage!;
    useComposerDraftStore.persist.setOptions({
      storage: {
        getItem: (name) => originalStorage.getItem(name),
        setItem: () => {},
        removeItem: (name) => originalStorage.removeItem(name),
      },
    });
    try {
      store.addTerminalContexts(
        threadRef,
        [
          makeTerminalContext({
            id: "dropped-context",
      ...
      expect(persistComposerDraftContentNow(threadRef, { prompt, attachments: [] })).toBe(false);
    } finally {
