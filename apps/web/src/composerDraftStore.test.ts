...
describe("composerDraftStore moveComposerPromptAndImages", () => {
  ...
  it("carries the input origin with the moved prompt and drains the source", () => {
    const store = useComposerDraftStore.getState();
    store.setPrompt(sourceDraftId, "read this back to me", "voice-transcription");

    store.moveComposerPromptAndImages(sourceDraftId, destinationDraftId);

    expect(draftByKey(destinationDraftId)?.inputOrigin).toBe("voice-transcription");
    // The drained source keeps no origin, so nothing pins an otherwise empty draft.
    expect(draftByKey(sourceDraftId)).toBeUndefined();
  });
});
