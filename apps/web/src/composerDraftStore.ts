        ...
        moveComposerPromptAndImages: (from, to) => {
          ...
          set((state) => {
            ...
            const nextDestination: ComposerThreadDraftState = {
              ...destination,
              prompt: movedPrompt,
              // The origin describes how the moved prompt was written, so it
              // travels with the text rather than staying on the drained
              // source. The destination is only ever written to when it holds
              // no user content, so its own origin cannot be lost here.
            ...
            const { inputOrigin: _movedInputOrigin, ...sourceWithoutInputOrigin } = source;
            const nextSource: ComposerThreadDraftState = {
              ...sourceWithoutInputOrigin,
