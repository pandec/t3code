...
const PersistedDraftThreadState = Schema.Struct({
  ...
  customGroupId: Schema.optionalKey(Schema.NullOr(Schema.String)),
 ...
 * sidebar draft rows (which draft sessions deserve a row), by new-thread
 * resurrection (a draft with content keeps its settings instead of being
 * reset to defaults), by the chat route's archive-undo empty-draft check,
 * and by the composer's unsaved-content guard.
 */
...
export interface DraftSessionState {
  ...
  /** Custom thread group the thread joins on first send; null or absent is Active. */
...
interface ComposerDraftStoreState {
  ...
  /** Transient route guard; deliberately omitted from persisted state. */
...
function createDraftThreadState(
  ...
  return {
    ...
    // Groups span environments, so the choice survives project changes.
    customGroupId:
      options?.customGroupId === undefined
        ? (existingThread?.customGroupId ?? null)
        : options.customGroupId,
    promotedTo: null,
  };
}
...
function draftThreadsEqual(left: DraftThreadState | undefined, right: DraftThreadState): boolean {
  return (
    ...
    (left.customGroupId ?? null) === (right.customGroupId ?? null) &&
  ...
  if (rawDraftThreadsByThreadId && typeof rawDraftThreadsByThreadId === "object") {
    for (const [threadKeyOrId, rawDraftThread] of Object.entries(
      ...
      draftThreadsByThreadKey[threadKey] = {
        ...
        startFromOrigin,
        ...(typeof candidateDraftThread.customGroupId === "string" &&
        candidateDraftThread.customGroupId.length > 0
          ? { customGroupId: candidateDraftThread.customGroupId }
          : {}),
...
function toHydratedDraftThreadState(
  ...
  return {
    ...
    ...(persistedDraftThread.customGroupId != null
      ? { customGroupId: persistedDraftThread.customGroupId }
      : {}),
        ...
        setDraftThreadContext: (threadRef, options) => {
          ...
          set((state) => {
            ...
            const isUnchanged =
              ...
              nextDraftThread.customGroupId === (existing.customGroupId ?? null) &&
...
export function persistComposerDraftContentNow(
  ...
  try {
    ...
    // Context records ride the same write as the prompt: a queued message's
    // links are only worth keeping if the payloads behind them landed too.
    const sameIds = (
      left: ReadonlyArray<{ readonly id: string }> | undefined,
      right: ReadonlyArray<{ readonly id: string }>,
    ) => {
      const leftIds = (left ?? []).map((item) => item.id);
      return (
        leftIds.length === right.length && right.every((item, index) => leftIds[index] === item.id)
      );
    };
    return (
      ...
      sameIds(persistedDraft.terminalContexts, current.terminalContexts) &&
      sameIds(persistedDraft.reviewComments, current.reviewComments) &&
      sameIds(persistedDraft.previewAnnotations, current.previewAnnotations)
    );
  } catch {
    return false;
  }
