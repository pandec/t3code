    ...
    (patch: UnifiedSettingsPatch) => {
      ...
      if (Object.keys(serverPatch).length > 0) {
        ...
        if (Object.keys(sharedPatch).length > 0) {
          ...
          // Only warn when every reachable target rejected the keys on
          // capability grounds. No targets at all is not a save failure here:
          // the local-patch branch above already covers the missing-server case.
          if (!wroteToTarget && targets.size > 0) {
            toastManager.add({
              type: "warning",
              title: "Setting not saved",
              description: "Update older servers to save this setting.",
            });
          }
        }
      }
