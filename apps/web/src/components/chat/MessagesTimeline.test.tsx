...
/** Assistant entry for cases that only care about the rendered body. */
function buildAssistantMessageEntry(text: string) {
...
describe("MessagesTimeline", () => {
  ...
  it("renders elapsed time at assistant response text size", () => {
  ...
  it("anchors the first user message using its measured height", () => {
    ...
    // `size` tracks !liveFollowEnabled (upstream hardcodes true): while
    // following, size compensation must not fight the end-scroll.
  ...
  it("disables tail following and preserves measured content position when detached", () => {
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        liveFollowEnabled={false}
  ...
  it("continues to render sanitized raw HTML in assistant messages", async () => {
    ...
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        timelineEntries={[
          buildAssistantMessageEntry("<details><summary>More</summary>Details</details>"),
        ]}
      />,
    );
  ...
  it.each(
    ...
    async ({ markdown, expected, strongCount, streaming }) => {
      ...
      const thought = buildAssistantTimelineEntry("thought", markdown, turnId);
      let renderer: ReactTestRenderer | undefined;
      try {
        await act(() => {
          renderer = create(
