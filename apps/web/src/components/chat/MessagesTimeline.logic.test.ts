...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps both minimap rails on compacted top-level message rows", () => {
    const firstTurnId = TurnId.make("turn-minimap-1");
    const secondTurnId = TurnId.make("turn-minimap-2");
    const timelineEntries = [
      {
        id: "user-minimap-1-entry",
        kind: "message" as const,
        createdAt: "2026-01-01T00:00:00Z",
        message: {
          id: MessageId.make("user-minimap-1"),
          role: "user" as const,
          text: "First prompt",
          turnId: null,
          createdAt: "2026-01-01T00:00:00Z",
      ...
      ...Array.from({ length: 3 }, (_, index) => ({
        id: `work-minimap-${index + 1}-entry`,
        kind: "work" as const,
        createdAt: `2026-01-01T00:00:0${index + 1}Z`,
        entry: {
          id: `work-minimap-${index + 1}`,
          createdAt: `2026-01-01T00:00:0${index + 1}Z`,
          label: "Ran command",
          tone: "tool" as const,
          toolLifecycleStatus: "completed" as const,
      ...
      {
        id: "assistant-minimap-1-entry",
        kind: "message" as const,
        createdAt: "2026-01-01T00:00:04Z",
        message: {
          id: MessageId.make("assistant-minimap-1"),
          role: "assistant" as const,
          text: "First response",
          turnId: firstTurnId,
          createdAt: "2026-01-01T00:00:04Z",
      ...
      {
        id: "user-minimap-2-entry",
        kind: "message" as const,
        createdAt: "2026-01-01T00:00:05Z",
        message: {
          id: MessageId.make("user-minimap-2"),
          role: "user" as const,
          text: "Second prompt",
          turnId: null,
          createdAt: "2026-01-01T00:00:05Z",
      ...
      {
        id: "assistant-minimap-2-entry",
        kind: "message" as const,
        createdAt: "2026-01-01T00:00:06Z",
        message: {
          id: MessageId.make("assistant-minimap-2"),
          role: "assistant" as const,
    ...
    const rows = deriveMessagesTimelineRows({
      timelineEntries,
      expandedWorkGroupIds: new Set(["work-group:work-minimap-1-entry"]),
      completedTurnAssistantMessageIds: new Set([
        MessageId.make("assistant-minimap-1"),
        MessageId.make("assistant-minimap-2"),
      ]),
      isWorking: false,
      activeTurnStartedAt: null,
      turnDiffSummaryByAssistantMessageId: new Map(),
    ...
    expect(rows.map((row) => row.id)).toEqual([
      "user-minimap-1-entry",
      "work-toggle:work-minimap-1-entry",
      "work-group:work-minimap-1-entry:details",
      "assistant-minimap-1-entry",
      "user-minimap-2-entry",
      "assistant-minimap-2-entry",
    ]);
    expect(
      deriveTimelineMinimapItems(rows, "user-turn").map(({ id, rowIndex }) => ({ id, rowIndex })),
    ).toEqual([
      { id: "user-minimap-1-entry", rowIndex: 0 },
      { id: "user-minimap-2-entry", rowIndex: 4 },
    ]);
    expect(
      deriveTimelineMinimapItems(rows, "final-assistant").map(({ id, rowIndex }) => ({
        id,
        rowIndex,
      })),
    ).toEqual([
      { id: "assistant-minimap-1-entry", rowIndex: 3 },
      { id: "assistant-minimap-2-entry", rowIndex: 5 },
    ]);
  });
