...
export interface ChatComposerHandle {
  ...
  /**
   * Restore context records behind a prompt's links (queued-message edit).
   * Returns the id rewrites the caller must apply to the prompt's references;
   * attachments are restored by the caller, so their records map to themselves.
   */
...
export interface ChatComposerProps {
  ...
  // Context window
  //
  // The fork derives the snapshot inside the composer rather than taking
  // upstream's hoisted `activeContextWindow` prop: its fallback to an empty
  // snapshot needs the picker's own `selectedModelSelection`, which is
  // composer-local state, and the provider-usage rings read the same
  // activity list.
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  // The RPC answer is authoritative for the thread's cwd; before it lands,
  // upstream's status-side workspace snapshot (falling back to machine
  // skills) is the closest per-cwd answer available.
  ...
  // The disabled picker shows the draft's own pick while the catalog loads,
  // so the label does not snap to it once the catalog lands.
  const pendingCatalogDraftInstanceId = composerDraft.activeProvider ?? undefined;
  const pendingCatalogPickerSelection = {
    instanceId:
      pendingCatalogDraftInstanceId ?? activeThreadModelSelection?.instanceId ?? selectedInstanceId,
    model:
      (pendingCatalogDraftInstanceId === undefined
        ? undefined
        : composerDraft.modelSelectionByProvider[pendingCatalogDraftInstanceId]?.model) ??
  ...
  // Context window
  // ------------------------------------------------------------------
  // The reported snapshot, before the fork's empty-state fallback. Compaction
  // reads this one: a thread that has never reported usage has nothing to
  // compact, and the fallback below would otherwise offer the action on a
  // synthesized zero-token reading.
  const reportedContextWindow = useMemo(
    () => deriveLatestContextWindowSnapshot(activeThreadActivities ?? []),
    [activeThreadActivities],
  );
  const emptyContextWindowSnapshot = useCallback(
  ...
  const activeContextWindow =
    ...
    (contextWindowDetailPending ? null : emptyContextWindowSnapshot(selectedModelSelection));
  ...
  const [isDesktopVoiceRecorderActive, setIsDesktopVoiceRecorderActive] = useState(false);
  const [isComposerFooterCompact, setIsComposerFooterCompact] = useState(false);
  const [isComposerPrimaryActionsCompact, setIsComposerPrimaryActionsCompact] = useState(false);
  const [isComposerModelPickerOpen, setIsComposerModelPickerOpen] = useState(false);
  // Non-null while a slash-command picker (the /t3-wait thread picker or the
  // /prompt saved-prompt picker) is open: the prompt offset the picked
  // insertion lands at (the cleared trigger's position) together with the
  // prompt it was computed against, so a prompt that mutated meanwhile falls
  // back to the live cursor instead of landing the insertion mid-word. One
  // state for both pickers — two could never be open at once, and a single
  // value makes that impossible to misrepresent.
  const [insertionPicker, setInsertionPicker] = useState<{
    kind: "thread-wait" | "saved-prompt";
    offset: number;
    promptAtOpen: string;
  } | null>(null);
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "skill") {
      return searchProviderSkills(effectiveProviderSkills, composerTrigger.query).map((skill) => ({
        id: `skill:${selectedProvider}:${skill.name}`,
        type: "skill" as const,
        provider: selectedProvider,
        skill,
  ...
  const onSelectComposerItem = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "slash-command") {
        ...
        if (item.command === "t3-wait" || item.command === "prompt") {
          const applied = applyPromptReplacement(trigger.rangeStart, trigger.rangeEnd, "", {
            expectedText: snapshot.value.slice(trigger.rangeStart, trigger.rangeEnd),
            focusEditorAfterReplace: false,
          });
          if (applied) {
            setComposerHighlightedItemId(null);
            setInsertionPicker({
              kind: item.command === "t3-wait" ? "thread-wait" : "saved-prompt",
  ...
  const insertFromPicker = useCallback(
    (kind: "thread-wait" | "saved-prompt", text: string) => {
      if (insertionPicker === null || insertionPicker.kind !== kind) return;
      const snapshot = readComposerSnapshot();
      // The stored offset and applyPromptReplacement both index the expanded
      // (raw) prompt text, so the stale-draft fallback must use the expanded
      // cursor — the collapsed one counts inline tokens as one character.
      const insertionOffset =
        snapshot.value === insertionPicker.promptAtOpen
          ? Math.min(insertionPicker.offset, snapshot.value.length)
          : Math.min(snapshot.expandedCursor, snapshot.value.length);
      setInsertionPicker(null);
      applyPromptReplacement(insertionOffset, insertionOffset, text, { expectedText: "" });
    },
    [applyPromptReplacement, insertionPicker, readComposerSnapshot],
  );

  const onThreadWaitPick = useCallback(
    (entry: ThreadWaitPickerEntry) =>
      insertFromPicker("thread-wait", buildThreadWaitInsertionText(entry)),
    [insertFromPicker],
  );

  const onSavedPromptPick = useCallback(
    (prompt: SavedPrompt) => insertFromPicker("saved-prompt", prompt.content),
    [insertFromPicker],
  );

  const onInsertionPickerClose = useCallback(() => {
    setInsertionPicker(null);
    composerEditorRef.current?.focusAt(composerCursor);
  }, [composerCursor]);

  // An approval taking over the composer, a reappearing trigger menu, or the
  // stash menu opening unmounts the picker; strand no insertion target
  // behind any of those gates.
  useEffect(() => {
    if (composerMenuOpen || isComposerApprovalState || isStashMenuOpen) {
      setInsertionPicker(null);
    }
  }, [composerMenuOpen, isComposerApprovalState, isStashMenuOpen]);
  ...
  const onComposerCommandKey = (
    ...
    // editing, the escape hatch for a steer already on its way out. It wins
    // over prompt-history recall, which is the fallback when nothing is queued.
    ...
    const submissionIntent =
      key === "Enter"
        ...
        : null;
    if (submissionIntent === "alternate") {
      // Enter into a running turn steers by default; the modifier asks for
      // the other delivery, which in the durable outbox is "queue for later".
      submitComposer(undefined, { deliveryIntent: "queue", submissionIntent: "foreground" });
      return true;
    }
    if (submissionIntent) {
  ...
  const composerHasExpandedChrome =
    ...
    isStashMenuOpen ||
    (composerHasRunningActions && composerSendState.hasSendableContent) ||
    ...
    <>
      ...
      <ProviderModelPicker
        ...
        {...(routeKind === "draft" && supportsMultipleModels
          ...
          : {})}
        activeInstanceId={
          providerCatalogPending ? pendingCatalogPickerSelection.instanceId : selectedInstanceId
        }
        model={
          providerCatalogPending
            ? pendingCatalogPickerSelection.model
            : selectedModelForPickerWithCustomFallback
        }
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      importContextRecords: (records) => importContextRecords(records, null),
      getSendContext: () => ({
        prompt: promptRef.current,
                ...
                >
                  ...
                  {voiceTranscriptionAvailable ? (
                    <DesktopVoiceRecorder
                      ...
                      onActiveChange={setIsDesktopVoiceRecorderActive}
                      onTranscript={(text) => {
                        const nextPrompt =
