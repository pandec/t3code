...
function MarkdownDetails({
  ...
  return (
    <div className="chat-markdown-details my-2 border-y border-border/60">
      <Collapsible
        defaultOpen={open}
...
function useChatMarkdownState({
  ...
  // The thread's own environment must win, and there is deliberately no
  // fall-back to the globally active one: that singleton tracks the routed
  // thread, which is the wrong server for markdown rendered in the secondary
  // split pane (file links would open via another machine's editor and RPC).
  // Surfaces with no thread pass `environmentId` explicitly instead.
...
function ChatMarkdown({
  ...
  const remarkPlugins = useMemo(
    ...
    [extraRemarkPlugins, incrementalParsing, linearRemarkPlugins, lineBreaks],
  );
