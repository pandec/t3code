...
import { ArchiveIcon, ChevronDownIcon, ImportIcon, PlusIcon, Trash2Icon } from "lucide-react";
...
function ProjectDetail({
  ...
  const accentColor = projectAccentColors.resolve(group.memberProjects);
  const updateProjectAccentColor = useCallback(
    (color: SidebarProjectAccentColor | null) => {
      projectAccentColors.update(group.memberProjects, color);
    },
  ...
  // repo's t3.json value when present, otherwise the global setting. Stays a
  // neutral "Default" until the file query settles so the label never flips.
  const inheritedEnvModeText = inheritedEnvModeLabels({
    status: t3File.status,
    fileEnvMode: t3File.file?.defaultThreadEnvMode,
    globalEnvMode: scriptSettings.defaultThreadEnvMode,
  });
    ...
    async (members: ReadonlyArray<SidebarProjectGroupMember>) => {
      ...
      if (isWholeGroup) {
        if (hasOtherMembers) {
          ...
          // The stored scope and hidden filters keep intent for projects that
          // may come back (disconnects), but a deliberate removal must not
          // leave the deleted group's logical key pinning the sidebar filter.
          updateSidebarProjectFilters((current) => ({
            scopeKeys: current.scopeKeys?.filter((key) => key !== group.projectKey) ?? null,
            hiddenProjectKeys: current.hiddenProjectKeys.filter((key) => key !== group.projectKey),
          }));
    ...
    <>
      <SettingsPageContainer className="gap-6">
        <SettingsSection title="Project" hideTitle>
          ...
          <SettingsRow
            title="Thread accent"
            description="Adds a subtle transparent tint to this project's sidebar threads."
            control={
                ...
                <SelectPopup align="end" alignItemWithTrigger={false}>
                  <SelectItem value="inherit">
                    {group.memberProjects.length > 1
                      ...
                      : inheritedEnvModeText.inheritItem}
                  </SelectItem>
          ...
          <SettingsRow
            ...
            control={
              <Button
                size="xs"
                variant="outline"
                onClick={() => setSessionImportTarget(selectedCheckout)}
              >
        ...
        <SettingsSection title="Threads">
          <SettingsRow
            ...
            control={
              <Button variant="outline" onClick={() => void openArchivedThreads()}>
                <ArchiveIcon />
                View archived
              </Button>
            }
