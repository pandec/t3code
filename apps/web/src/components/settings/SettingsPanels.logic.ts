 ...
 * continuation group — so the settings card can warn while the user configures
 * rather than leaving a silent no-op at runtime. The driver check lives in the
 * caller, which only offers same-driver candidates.
 *
