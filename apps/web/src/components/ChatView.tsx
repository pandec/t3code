  ...
  const updatePullRequestTabStatusFromPanel = useCallback(
    (status: PullRequestTabStatus) => {
      ...
      setPullRequestTabStatuses((current) => {
        ...
        if (
          previous?.projectId === status.projectId &&
          previous.repository === status.repository &&
          previous.number === status.number &&
          previous.state === status.state &&
          previous.isDraft === status.isDraft
        ) {
          return current;
        }
        return { ...current, [activePullRequestSurfaceId]: status };
      });
    },
