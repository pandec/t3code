...
function OpenCommandPaletteDialog(props: {
  ...
  actionItems.push(
    ...buildThreadCopyActionItems({
      threadId: copyThreadIdTarget,
      reference: activeThreadReferenceCopyTarget,
      icon: <LinkIcon className={ITEM_ICON_CLASS} />,
      copy: copyActiveThreadReference,
    }),
  );
  ...
  function handleKeyDown(event: KeyboardEvent<HTMLInputElement>): void {
    ...
    if (command === "thread.copyId") {
      event.preventDefault();
      event.stopPropagation();
      if (copyThreadIdTarget === null || event.repeat) return;
      setOpen(false);
      void copyActiveThreadReference(
        resolveThreadReferenceCopyTarget({ threadId: copyThreadIdTarget }),
      );
      return;
    }
