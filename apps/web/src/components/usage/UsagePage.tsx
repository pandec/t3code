...
import type { PartialUsageSource } from "@t3tools/shared/usageMerge";
import {
  enumerateDays,
  formatCount,
  formatDayShort,
...
export function UsagePage() {
  ...
  // The window lives in state rather than being derived at render time, so a
  // re-render cannot shift the range and refetch every environment. Selecting
  // a length or refreshing recomputes it explicitly, so a refresh after
  // midnight includes the new day.
  const [window, setWindow] = useState(() => makeWindow(DEFAULT_WINDOW_DAYS));
  const { merged, environments, isPending, isPartial, refresh: rescan } = useUsage(window);

  const selectWindow = (days: number) => {
    setWindowDays(days);
    setWindow(makeWindow(days));
  };

  const refresh = () => {
    const next = refreshWindow(window, windowDays);
    // Same range: rescan the existing queries in place. New range (the clock
    // crossed midnight): the new window's queries fetch fresh on mount.
    if (next === window) {
      rescan();
    } else {
      setWindow(next);
  ...
  return (
    <div className="flex flex-col gap-1 border border-border px-3 py-2 text-xs text-muted-foreground">
      ...
      {partialSources.map((source) => (
        <span key={`${source.environmentId}:${source.provider}:${source.resolvedHomePath}`}>
          {source.label} · {PROVIDER_LABEL[source.provider]}:{" "}
          {source.message ?? "Usage is incomplete."}
        </span>
      ))}
      {duplicateSources.length > 0 ? (
        <span>
