...
interface EarlyDesktopSettingsInput {
  ...
  /** Packaged Dev builds use the development identity without a dev server. */
...
export interface EarlyLinuxElectronOptions {
  ...
  /** Dev server OR packaged Dev: the identity that picks scheme, entry, and state paths. */
...
/** Mirrors `DesktopEnvironment`'s `usesDevelopmentIdentity` before app services exist. */
const usesDevelopmentIdentity = (input: EarlyDesktopSettingsInput): boolean =>
  isDevelopmentEnvironment(input.env) || input.isPackagedDevBuild === true;
...
function resolveEarlyDesktopSettingsPath(input: EarlyDesktopSettingsInput): string {
  ...
  const stateDir = resolveDesktopStateDir({
    ...
    isPackagedDevBuild: input.isPackagedDevBuild === true,
    joinPath: input.joinPath,
    t3Home,
  });
...
export function resolveEarlyLinuxElectronOptions(
  ...
  return {
    isDevelopment,
    usesDevelopmentIdentity: usesDevelopmentIdentity(input),
    linuxWmClass: usesDevelopmentIdentity(input) ? "t3code-dev" : "t3code",
    linuxDesktopEntryName: resolveLinuxDesktopEntryName(usesDevelopmentIdentity(input)),
