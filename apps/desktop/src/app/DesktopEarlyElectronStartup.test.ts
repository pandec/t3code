...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("gives a packaged Dev build the development identity without a dev server", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: { XDG_CURRENT_DESKTOP: "niri" },
      homeDirectory: "/home/user",
      isPackagedDevBuild: true,
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.t3/dev-packaged/desktop-settings.json");
        return JSON.stringify({ linuxPasswordStore: "auto" });
      },
    });
  ...
  it("keeps packaged Dev state separate even when T3CODE_HOME is configured", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { T3CODE_HOME: "/home/user/.t3-test" },
      homeDirectory: "/home/user",
      isPackagedDevBuild: true,
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.t3-test/dev-packaged/desktop-settings.json");
        return JSON.stringify({ linuxPasswordStore: "kwallet6" });
      },
    });
