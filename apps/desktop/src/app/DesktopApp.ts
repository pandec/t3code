...
const bootstrap = Effect.gen(function* () {
  ...
  yield* electronProtocol.registerDesktopProtocol({
    scheme: ElectronProtocol.getDesktopScheme(environment.usesDevelopmentIdentity),
...
const startup = Effect.gen(function* () {
  ...
  if (environment.usesDevelopmentIdentity && environment.isPackaged) {
    const disabledEnvName = "T3CODE_DESKTOP_APP_UPDATE_DISABLED";
    process.env[disabledEnvName] = "1";
    if (environment.platform === "win32") {
      process.env.WSLENV = process.env.WSLENV
        ? `${process.env.WSLENV}:${disabledEnvName}`
        : disabledEnvName;
    }
  }
