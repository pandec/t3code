...
describe("buildThreadSettingsMenu", () => {
  it("orders the top level as model, options, runtime, interaction", () => {
  ...
  it("summarizes the current choice on each submenu row", () => {
    ...
    expect(menu.actions.find((action) => action.title === "Interaction")?.subtitle).toBe("Default");
  });
  ...
  it("resolves option, runtime, and interaction events with checkmarked current values", () => {
    const menu = buildThreadSettingsMenu(baseInput());
    ...
    const interactionItems =
      menu.actions.find((action) => action.title === "Interaction")?.subactions ?? [];
    expect(interactionItems.find((action) => action.title === "Default")?.state).toBe("on");
    expect(eventFor(menu, interactionItems.find((action) => action.title === "Plan")?.id)).toEqual({
  ...
  it("registers an event for every leaf action id", () => {
    ...
    expect(eventTypes(menu)).toEqual(
      new Set(["select-model", "set-option", "set-runtime", "set-interaction"]),
    );
  });
});
