...
export type ThreadSettingsMenuEvent =
  ...
  | { readonly type: "set-interaction"; readonly mode: ProviderInteractionMode };
...
export function buildThreadSettingsMenu(input: {
  ...
  const interactionLabel = INTERACTION_MODE_CHOICES.find(
    (choice) => choice.mode === input.interactionMode,
  )?.label;
  actions.push({
    id: "interaction",
    title: "Interaction",
    ...(interactionLabel === undefined ? {} : { subtitle: interactionLabel }),
    subactions: INTERACTION_MODE_CHOICES.map((choice): MenuAction => {
      const id = `interaction:${choice.mode}`;
      events.set(id, { type: "set-interaction", mode: choice.mode });
      return {
        id,
        title: choice.label,
        state: choice.mode === input.interactionMode ? "on" : "off",
      };
    }),
  });
