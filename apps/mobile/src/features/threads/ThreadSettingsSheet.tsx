...
export function ThreadSettingsSheet(props: {
  ...
  // state would otherwise stick around — and the reset happens during render
  // (not in an effect) so the previous open's staged state can never paint
  // for a frame before being cleared.
  ...
  // Hosts can unmount a presented sheet outright (the new-task screen swaps
  // to its project picker when the selected project disappears). RN Modal
  // fires no onDismiss for that path, so report dismissal from the unmount
  // cleanup; the presentation hook's transition table makes duplicates and
  // not-yet-presented calls no-ops.
      ...
      : submenu?.kind === "interaction"
        ? {
            ...
            rows: INTERACTION_MODE_CHOICES.map((choice) => ({
              ...
              onPress: () => {
                void Haptics.selectionAsync();
                // Live like Runtime: interaction mode applies immediately,
                // independent of any staged model pick.
