...
export function NewTaskDraftScreen(props: {
  ...
  const overlayOwnerRef = useRef<"settings" | "attachment" | "preview" | "context" | null>(null);
  ...
  // shares the composer's single overlay owner with settings and strip previews.
  const acquireContextOverlay = useCallback(() => {
    if (overlayOwnerRef.current !== null) return false;
    overlayOwnerRef.current = "context";
    return true;
  }, []);
  const releaseContextOverlay = useCallback(() => {
    if (overlayOwnerRef.current === "context") overlayOwnerRef.current = null;
  }, []);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (overlayOwnerRef.current === "settings") overlayOwnerRef.current = null;
    }, [settingsSheetPresentation.onDismissed]),
  );
  useEffect(
    () =>
  ...
  const promptEditor = (
    <>
      <ComposerEditor
        ...
        onAcquireOverlay={acquireContextOverlay}
        onReleaseOverlay={releaseContextOverlay}
        onOpenMention={(path) => {
