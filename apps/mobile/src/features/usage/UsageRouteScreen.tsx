...
export function UsageRouteScreen() {
  ...
  // before. The initial scan renders its own placeholder, while unreachable
  // environments settle into coverage notices instead of pinning the spinner.
  const refreshing = environments.some(
    (entry) => entry.queryPending && entry.state.kind === "reported",
  );
  ...
  const refreshUsage = () => {
    const nextWindow = refreshUsageWindow(window, windowDays);
  ...
  return (
    <View collapsable={false} className="flex-1 bg-sheet">
      ...
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshUsage} />}
      >
        <SegmentedControl
...
/**
 * Says plainly when totals are incomplete: an environment still answering,
 * one that failed or is unreachable, or one whose transcripts another
...
function UsageCoverageNotice(props: {
  ...
  const failed = props.environments.filter((environment) => environment.state.kind === "failed");
  ...
  const partialSources = props.merged.partialSources;
  if (
    failed.length === 0 &&
  ...
  return (
    <View className="gap-1 rounded-[16px] border-continuous bg-card px-4 py-3">
      ...
      {partialSources.map((source) => (
        <Text
          key={`${source.environmentId}:${source.provider}:${source.resolvedHomePath}`}
