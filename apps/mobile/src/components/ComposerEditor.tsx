...
export type ComposerEditorProps = NativeComposerEditorProps & {
  ...
  /**
   * The context sheet (and the media or video preview it presents directly) is a composer
   * overlay, so it competes with the settings, usage and preview overlays for the composer.
   * `onAcquireOverlay` runs synchronously before the sheet is shown and returns false when
   * another overlay already owns the composer, in which case the tap is dropped.
   * `onReleaseOverlay` runs on every close path of that one sheet lifecycle.
   */
  readonly onAcquireOverlay?: () => boolean;
  readonly onReleaseOverlay?: () => void;
};
...
export function ComposerEditor({
  ...
  const releaseOverlayRef = useRef(onReleaseOverlay);
  releaseOverlayRef.current = onReleaseOverlay;
  const closeSelected = () => {
    if (selectedRef.current === null) return;
    selectedRef.current = null;
    setSelected(null);
    releaseOverlayRef.current?.();
  };
  // A sheet still open when the draft changes underneath it or the editor unmounts would
  // otherwise keep the composer's overlay owner forever.
  useEffect(() => () => closeSelected(), [draftKey]);
    ...
    <>
      <NativeComposerEditor
        ...
        onContextPress={(selection) => {
          ...
          // Only a chip the sheet can present takes the overlay; an unknown source would
          // acquire the owner with nothing on screen to release it.
          const presentable =
            collectComposerContextReferences(selection.source).length > 0 ||
            (selection.source.startsWith("$") &&
              props.skills?.some((skill) => skill.name === selection.source.slice(1)) === true);
          if (!presentable || selectedRef.current !== null) return;
          if (onAcquireOverlay && !onAcquireOverlay()) return;
