...
describe("buildThreadFeed", () => {
  ...
  it("keeps plan updates visible in the timeline", () => {
    const thread = makeThread({
      id: ThreadId.make("thread-plan-update"),
      projectId: ProjectId.make("project-1"),
      title: "Plan update",
      activities: [
        makeActivity({
          id: EventId.make("plan-update"),
          kind: "turn.plan.updated",
          tone: "info",
          summary: "Plan updated",
          createdAt: "2026-09-01T00:00:00.000Z",
          turnId: TurnId.make("turn-plan-update"),
          payload: {
            explanation: "Implement the change",
            plan: [{ step: "Update the feed", status: "inProgress" }],
          },
        }),
      ],
    });
    ...
    expect(buildThreadFeed(thread)).toMatchObject([
      {
        type: "activity-group",
        activities: [{ id: "plan-update", summary: "Plan updated" }],
      },
    ]);
  });
  ...
  it("measures a steer-superseded turn from its user boundary through trailing work", () => {
    ...
    expect(expanded.map((entry) => entry.id)).toEqual([
      "user-1",
      "turn-fold:turn-1",
      "work-toggle:work-group:work-before-response",
      "assistant-commentary",
      "work-toggle:work-group:work-after-response",
      "user-2",
      "assistant-next",
    ]);
  });
