...
const appVersion = repoEnv.T3CODE_FORK_VERSION?.trim() || "1.2.0";
...
const config: ExpoConfig = {
  ...
  ios: {
    ...
    // Keyed to the resolved identifier so custom-team and personal builds
    // provision a group they can actually sign, not upstream's bundle id.
    entitlements: {
  ...
  plugins: [
    ...
    [
      "expo-audio",
      {
        microphonePermission: "Allow T3 Code to record voice prompts for transcription.",
      },
    ],
    [
      "expo-font",
    ...
    ...(isIosPersonalTeamBuild
      ? ["./plugins/withoutIosPersonalTeamCapabilities.cjs", sharingPlugin]
    ...
    [
      "expo-build-properties",
      {
        ...
        ios: {
          ...
          // core. If that artifact probe falls back to source, the resulting
          // app can omit dynamic React frameworks required by the Expo binaries.
          // Build Expo modules from source so either React path stays coherent.
