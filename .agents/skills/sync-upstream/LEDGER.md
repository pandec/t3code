...
- **Mobile v2 pending rows keep fork filters** (2026-07-29). Upstream's shared `buildThreadListV2ListItems` owns active → pending → settled ordering and pending-row metadata. Before that splice, both `HomeScreen.tsx` and `ThreadNavigationSidebar.tsx` must retain the fork's environment, model, project, and search predicates, and rows must keep `resolveThreadProviderDriver` so the same environment config source drives model labels, icons, and capability gates.
