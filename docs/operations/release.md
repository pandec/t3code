...
- Triggers:
  - manual `workflow_dispatch` with `channel=stable`, the normal way to ship stable. Stable
    and nightly dispatches must select the repository default branch; preview may select any branch. The channel defaults
...
the **Update** action targeting a package version that does not exist yet.
