...
Therefore **handover = transfer the provider file(s) + run the existing session-import** (`apps/server/src/sessionImport/SessionImportService.ts`, RPC `sessionImport.listCandidates` / `sessionImport.import`) on the target. T3 database rows are never moved — they are environment-scoped (environment id, auth tables, `revision`, `provider_instance_id`) and threads are intentionally environment-local (`docs/internals/remote.md:180-189`).
...
1. **`t3 session import --file <transcript> --project <id-or-path> [--worktree-branch B] [--model M] [--effort E] [--instance I] [--title T] [--json]`**
   ...
   - Uses authenticated HTTP session-import endpoints while retaining the existing binding-first transaction and deterministic duplicate recovery. Existing live bindings no longer hide a local candidate: candidate reads attach its owning thread metadata, while missing or deleted thread projections are treated as stale. Re-import is rejected (retryable) while the stale thread's provider session is still shutting down.
   - `--title` carries a T3 thread title across a handover (T3 titles live in the database and never travel inside a transcript); without it an explicit provider name wins, then Claude's generated `ai-title`, then the first importable user message.
...
2. **`t3 session candidates --project <id-or-path> [--cwd <worktree>] [--json]`** — standalone candidate inspection for a project or validated existing worktree. Sessions already attached to a T3 Code thread remain visible with `linkedThread` metadata (and a human-readable `linked:<thread-id>` marker), including sessions from threads created inside T3 Code at that workspace root. An explicit re-import with fork permission requires the source provider session to be idle (a busy source is rejected, retryable) and copies its latest history into a fresh native continuation instead of sharing the original id between two threads; the new thread uses the normal forked-thread title convention.
