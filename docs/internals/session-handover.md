...
2. **`t3 session candidates --project <id-or-path> [--cwd <worktree>] [--json]`** — standalone candidate inspection for a project or validated existing worktree. Already-imported sessions remain visible with `linkedThread` metadata (and a human-readable `linked:<thread-id>` marker); an explicit re-import with fork permission copies the original session's latest history into a fresh native continuation instead of sharing the original id between two threads.
