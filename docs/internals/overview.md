...
Four queue-backed workers handle provider ingestion, provider commands, checkpoints, and thread
settlement. `MessageSpeechReactor` is the fifth drainable service. It runs different messages
concurrently, serializes jobs for each message, and exposes `drain` like the queue-backed workers.
