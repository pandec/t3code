...
Use **By subscription** to credit activity to the subscription it consumes. This includes
gateway routing across providers: Claude Code routed to an OpenAI model counts toward Codex, and
Codex routed to an Anthropic model counts toward Claude Code. Use **By app** to credit the provider
app that produced each session instead.

OpenCode reads its SQLite database and older JSON history. Antigravity reads local conversation
databases. These readers use the server’s environment, not individual account settings. Set `OPENCODE_DATA_DIR` or `ANTIGRAVITY_DATA_DIR` on the
...
For Claude Code, Codex, and Grok, Usage includes each configured account's history, including disabled accounts. Custom homes follow
