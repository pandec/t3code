...
commands, and skills from that local installation. Discovery opens a short ACP session, so it
re-runs once a day rather than on the shared provider-health interval; changing Hermes settings
or pressing Refresh in provider settings re-runs it immediately. Hermes authentication remains owned by the CLI.
