...
`streaming: true` while the assistant is still writing it. Messages with inline context also
include the optional `context` object, containing its version and context records. Unlike the
