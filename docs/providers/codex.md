...
error — new turns route to the failover instance until the limit lifts, then return. Live threads
follow only when the providers share the same `CODEX_HOME path` (the shadow-home setup above).
