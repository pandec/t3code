...
/**
 * Recomputes the window for the same length, returning `current` itself when
 * the calendar range is unchanged. Refresh handlers can then compare by
 * identity: same object means rescan in place, a new object means the clock
 * crossed midnight and the window now includes the new day.
 */
export function refreshWindow(
  current: UsageSummaryInput,
  days: number,
  now = new Date(),
): UsageSummaryInput {
  const next = makeWindow(days, now, current.resolution);
  return next.sinceDay === current.sinceDay &&
    next.untilDay === current.untilDay &&
    next.timeZone === current.timeZone &&
    next.resolution === current.resolution &&
    next.sinceTime === current.sinceTime &&
    next.untilTime === current.untilTime
    ? current
    : next;
}
