...
describe("mergeUsage", () => {
  ...
  it("excludes legacy provider totals when directory ownership is split", () => {
    const shared = { provider: "claude" as const, hostId: "host", homePath: "/shared" };
    const unique = { ...shared, homePath: "/unique" };
    const legacy = environment(
      "legacy",
      summary(
        [bucket({ costUsd: 100, model: "gpt-5.6-sol" })],
        [shared, unique],
        USAGE_MERGE_COMPATIBLE_SINCE,
      ),
    );
    const current = environment("current", {
      ...summary([bucket({ sourcePath: "/shared", costUsd: 10, model: "gpt-5.6-sol" })], [shared]),
    ...
    for (const attribution of ["pool", "source"] as const) {
      for (const environments of [
        ...
        const merged = mergeUsage(environments, USAGE_CONTRACT_VERSION, { attribution });
        expect(merged.costUsd).toBe(10);
        expect(merged.contributingEnvironments).toEqual(["current"]);
        expect(merged.partialSources).toEqual([
          expect.objectContaining({
            environmentId: "legacy",
            provider: "claude",
            resolvedHomePath: "/unique",
            message: expect.stringContaining("overlapping totals are excluded"),
          }),
        ]);
      }
    }
    const owned = mergeUsage([legacy], USAGE_CONTRACT_VERSION);
    expect(owned.costUsd).toBe(100);
    expect(owned.partialSources).toEqual([]);
  });
  ...
  it("prefers the newest scan even when it is partial", () => {
    const source = { provider: "claude" as const, hostId: "mac", homePath: "/home/theo/.claude" };
    ...
    for (const ordered of [
      ...
      expect(merged.partialSources.map((source) => source.environmentId)).toEqual(["new"]);
      expect(merged.contributingEnvironments).toEqual(["new"]);
      expect(merged.duplicateSources).toEqual(["old: /home/theo/.claude"]);
    }
    expect(mergeUsage([partial], USAGE_CONTRACT_VERSION).costUsd).toBe(4);
  });

  it("uses only the newest source owner, including its new cells", () => {
    const source = { provider: "claude" as const, hostId: "mac", homePath: "/home/theo/.claude" };
  ...
  it("uses newest partial totals rather than an older complete cell", () => {
    const source = { provider: "claude" as const, hostId: "mac", homePath: "/home/theo/.claude" };
    const complete = environment("old", summary([bucket()], [source]));
    const partialSummary = summary(
      [
    ...
    expect(merged.costUsd).toBe(4);
    expect(merged.partialSources.map((source) => source.environmentId)).toEqual(["new"]);
    expect(merged.totalTokens).toBe(620);
  ...
  it("reports only owned partial sources from current servers", () => {
    ...
    const merged = mergeUsage(
      [
        ...
        environment("env-c", summary([bucket()], [partial], USAGE_MERGE_COMPATIBLE_SINCE - 1)),
    ...
    expect(merged.partialSources).toEqual([
      {
        ...
        label: "env-d",
        provider: "codex",
        resolvedHomePath: "/home/theo/.codex",
        message: "1 transcript file could not be read; usage from it is missing.",
      },
    ]);
  });
