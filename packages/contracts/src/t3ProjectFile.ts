...
export const T3ProjectFile = Schema.Struct({
  ...
  defaultThreadEnvMode: Schema.optionalKey(
    ThreadEnvMode.annotate({
      description:
        'Where new threads start for this repository: "worktree" for a fresh git worktree, "local" for the current checkout. Project and environment settings in T3 Code override this; when all are unset, the current checkout is used.',
    }),
  ),
