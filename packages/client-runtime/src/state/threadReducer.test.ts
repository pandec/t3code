...
describe("applyThreadDetailEvent", () => {
  ...
  describe("thread.created", () => {
    it.each([undefined, null, "research"])(
      "creates a fresh thread in group %s",
      (customGroupId) => {
        const result = applyThreadDetailEvent(baseThread, {
          ...baseEventFields,
          sequence: 1,
        ...
        if (result.kind === "updated") {
          ...
          expect(result.thread.customGroupId).toBe(customGroupId ?? null);
  ...
  describe("thread.activity-appended", () => {
    ...
    it("retains in-order appends across the 500-row boundary", () => {
      const makeActivity = (sequence: number) => ({
      ...
      const append = (thread: OrchestrationThread, sequence: number) =>
        applyThreadDetailEvent(thread, {
          ...
          payload: {
            threadId: ThreadId.make("thread-1"),
            activity: makeActivity(sequence),
          },
        });
      ...
      const first = append(
        {
          ...baseThread,
          activities: Array.from({ length: 499 }, (_, index) => makeActivity(index)),
        },
        499,
      );
      expect(first.kind).toBe("updated");
      ...
      const second = append(first.thread, 500);
      expect(second.kind).toBe("updated");
      if (second.kind !== "updated") return;
      expect(second.thread.activities).toHaveLength(500);
      expect(second.thread.activities[0]?.id).toBe("activity-1");
      expect(second.thread.activities.at(-1)?.id).toBe("activity-500");

      const third = append(second.thread, 501);
      expect(third.kind).toBe("updated");
      if (third.kind === "updated") {
        expect(third.thread.activities).toHaveLength(500);
        expect(third.thread.activities[0]?.id).toBe("activity-2");
        expect(third.thread.activities.at(-1)?.id).toBe("activity-501");
      }
    });
