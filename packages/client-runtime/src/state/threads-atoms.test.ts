...
const makeHarness = Effect.fn("TestThreadAtoms.makeHarness")(function* (options?: {
  ...
  const details = createEnvironmentThreadDetailAtoms(raw.stateAtom, raw.loadOlderMessagesAtom);
...
describe("createEnvironmentThreadStateAtoms", () => {
  ...
  it("keeps atom identities stable while live atoms remain collectible", () => {
    ...
    expect(loadOlderMessagesAtom.idleTTL).toBe(0);
  ...
  it.effect("keeps warm data and resumes a completed cursor while refreshing artifacts", () =>
    Effect.gen(function* () {
      const h = yield* makeHarness();
      const unmount = h.registry.mount(h.stateAtom);
      const first = yield* Queue.take(h.subscriptions);
      ...
      // The registry-local completed snapshot is authoritative during its warm TTL; only artifacts refresh over HTTP.
      expect(h.counts()).toEqual({ httpLoads: 2, diskLoads: 1, opened: 2, active: 1 });
      remount();
      yield* Deferred.await(next.closed);
    }),
  );
  ...
  it.effect("cancels older-page work on unmount and permits it again on a warm return", () =>
    Effect.gen(function* () {
      ...
      const unmount = h.registry.mount(h.stateAtom);
      const first = yield* Queue.take(h.subscriptions);
      h.registry.set(h.details.loadOlderMessagesAtom(h.ref), undefined);
