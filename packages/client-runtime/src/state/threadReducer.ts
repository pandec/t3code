  ...
  switch (event.type) {
    ...
    case "thread.activity-appended": {
      ...
      if (
        ...
        const appendedActivities = Arr.append(thread.activities, activity);
        const activities = retainRecentActivities(appendedActivities);
        activityIdIndex.delete(thread.activities);
        if (activities === appendedActivities) {
          ids.add(activity.id);
