...
describe("thread pagination state", () => {
  ...
  it.effect("keeps a new page loading when a snapshot replaced a parked older page", () =>
    Effect.gen(function* () {
      ...
      expect((yield* SubscriptionRef.get(harness.threadState.state)).olderMessages.isLoading).toBe(
        true,
      );
      ...
      yield* harness.awaitState(
        (value) =>
          ...
          Option.exists(value.page, (page) => page.beforeCursor === "cursor-2"),
      );
      ...
      const loading = yield* SubscriptionRef.get(harness.threadState.state);
      expect(loading.olderMessages.isLoading).toBe(true);
      expect(hasMessage(loading, "message-old")).toBe(false);
      expect(
        (yield* Ref.get(harness.loaderWindows)).map((window) =>
          window !== undefined && "beforeCursor" in window ? window.beforeCursor : undefined,
        ),
      ).toEqual([undefined, "cursor-1", "cursor-2"]);
