...
const decodeXcodeList = Schema.decodeUnknownEffect(
  Schema.fromJsonString(
    Schema.Struct({ workspace: Schema.Struct({ schemes: Schema.Array(Schema.String) }) }),
  ),
);
const decodeXcodeBuildSettings = Schema.decodeUnknownEffect(
  Schema.fromJsonString(
    Schema.Array(
      Schema.Struct({
        buildSettings: Schema.Struct({
          PRODUCT_BUNDLE_IDENTIFIER: Schema.optionalKey(Schema.String),
          TARGET_BUILD_DIR: Schema.optionalKey(Schema.String),
          FULL_PRODUCT_NAME: Schema.optionalKey(Schema.String),
          WRAPPER_EXTENSION: Schema.optionalKey(Schema.String),
        }),
      }),
    ),
  ),
...
const nativeIdentifier = Effect.fn("nativeClient.nativeIdentifier")(function* (
  ...
  const json = output
    .split("\n")
    .find((line) => line.startsWith("T3_NATIVE_CONFIG="))
    ?.slice("T3_NATIVE_CONFIG=".length);
  if (!json)
    return yield* new NativeClientError({ message: "Expo did not return the native app config." });
  const config = yield* decodeExpoConfig(json);
  return platform === "ios" ? config.ios.bundleIdentifier : config.android.package;
});

export function selectIosWorkspace(entries: ReadonlyArray<string>): string {
  const workspaces = entries.filter((entry) => entry.endsWith(".xcworkspace")).sort();
  if (workspaces.length !== 1)
    throw new NativeClientError({
      message: `Expected one generated iOS workspace, found ${workspaces.length}.`,
    });
  return workspaces[0]!;
}

export function selectIosScheme(workspace: string, schemes: ReadonlyArray<string>): string {
  const basename = workspace.slice(0, -".xcworkspace".length);
  return schemes.find((scheme) => scheme === basename) ?? (schemes.length === 1 ? schemes[0]! : "");
}

type IosBuildSettings = {
  readonly buildSettings: {
    readonly PRODUCT_BUNDLE_IDENTIFIER?: string;
    readonly TARGET_BUILD_DIR?: string;
    readonly FULL_PRODUCT_NAME?: string;
    readonly WRAPPER_EXTENSION?: string;
  };
};

export function selectIosAppPath(
  entries: ReadonlyArray<IosBuildSettings>,
  identifier: string,
): string | null {
  const apps = entries.filter(
    ({ buildSettings }) =>
      buildSettings.PRODUCT_BUNDLE_IDENTIFIER === identifier &&
      buildSettings.WRAPPER_EXTENSION === "app" &&
      buildSettings.TARGET_BUILD_DIR !== undefined &&
      buildSettings.FULL_PRODUCT_NAME?.endsWith(".app"),
  );
  if (apps.length !== 1) return null;
  const { TARGET_BUILD_DIR: directory, FULL_PRODUCT_NAME: product } = apps[0]!.buildSettings;
...
export const installedBinary = Effect.fn("installedBinary")(function* (
  ...
  if (platform === "ios") {
    ...
    if (!apps.includes(`"${identifier}"`)) return null;
    return yield* hashBundle(
      yield* run("xcrun", ["simctl", "get_app_container", device, identifier, "app"]),
    );
  }
  ...
  if (!installed.split("\n").some((line) => line.trim() === `package:${identifier}`)) return null;
  const packages = yield* run("adb", ["-s", device, "shell", "pm", "path", identifier]);
  const apks = packages
    .split("\n")
...
const main = Command.make(
  ...
  Effect.fn("nativeClient.main")(function* ({ mode, platform, device }) {
    ...
    const operations = {
      ...
      installedBinary: installedBinary(platform, device, identifier),
      ...
      build: Effect.gen(function* () {
        ...
        if (platform === "ios") {
          ...
          const iosDirectory = path.join(mobile, "ios");
          const workspaceName = selectIosWorkspace(yield* fs.readDirectory(iosDirectory));
          const workspace = path.join(iosDirectory, workspaceName);
          const listing = yield* decodeXcodeList(
            yield* command("xcrun", ["xcodebuild", "-list", "-json", "-workspace", workspace]),
          );
          const scheme = selectIosScheme(workspaceName, listing.workspace.schemes);
          if (!scheme)
            return yield* new NativeClientError({
              message:
                "Could not choose the generated iOS app scheme: expected the workspace name or one sole scheme.",
            });
          ...
          const settings = yield* command("xcrun", [
            "xcodebuild",
            "-showBuildSettings",
            "-json",
            "-workspace",
          ...
          const appPath = selectIosAppPath(yield* decodeXcodeBuildSettings(settings), identifier);
          if (!appPath)
            return yield* new NativeClientError({
              message: `Xcode did not report exactly one app product for ${identifier}.`,
            });
          yield* command("xcrun", ["simctl", "install", device, appPath], true);
        } else {
          yield* command(
            "vp",
            [
