...
const DesktopBuildFlavor = Schema.Literals(["release", "dev"]);
...
const BuildEnvConfig = Config.all({
  ...
  flavor: Config.schema(DesktopBuildFlavor, "T3CODE_DESKTOP_BUILD_FLAVOR").pipe(Config.option),
...
export const resolveBuildOptions = Effect.fn("resolveBuildOptions")(function* (
  ...
  const flavor = mergeOptions(input.flavor ?? Option.none(), env.flavor, "release");
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: isDevFlavor ? `${DESKTOP_APP_ID}.dev` : DESKTOP_APP_ID,
    productName: resolveDesktopProductName(version, flavor),
    artifactName: isDevFlavor
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      ...
      // each install in its own format. Dev builds ship the AppImage only: their
      // updater is disabled and a Dev .deb would share the release package name.
      target: target === "AppImage" && !isDevFlavor ? [target, "deb"] : [target],
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  if (!options.skipBuild) {
    ...
    yield* runCommand(
      ChildProcess.make(spawnCommand.command, spawnCommand.args, {
        ...
        shell: spawnCommand.shell,
        env: {
          T3CODE_DESKTOP_BUILD_FLAVOR: options.flavor,
        },
        extendEnv: true,
      }),
  ...
  const configuredMacPasskeySigning =
    options.platform === "mac" && options.signed && options.flavor === "release"
      ? yield* Effect.try({
  ...
  const stagePackageJson: StagePackageJson = {
    name: options.flavor === "dev" ? "t3code-dev" : "t3code",
    version: appVersion,
  ...
  if (options.platform === "win") {
    yield* validateWindowsPackagedPayload({
      stageDistDir,
      // The flavor has to travel here too: a Dev build ships
      // "T3 Code (Dev).exe", so the release-only name would fail validation.
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  flavor: Flag.Literals("flavor", DesktopBuildFlavor.literals).pipe(
    Flag.withDescription(
      "Desktop identity flavor, release or dev (env: T3CODE_DESKTOP_BUILD_FLAVOR).",
    ),
    Flag.optional,
  ),
