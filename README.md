...
- **Message queueing and steering** — on web and mobile, sending during a running turn steers at the agent's next step after a five-second recall window; web's “Queue for later” holds the message until the turn completes, and ArrowUp recalls the newest queued message into an otherwise empty composer. The visible outbox on both clients also lets queued messages be steered, edited, or deleted. A queued message whose images no longer resolve offers a recovery action instead of failing silently.
