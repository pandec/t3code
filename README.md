...
- **Archived-thread recents, search, grouping & project filtering** — the final Sidebar V2 section on desktop and mobile keeps a configurable recent archive close at hand, with one-action restore, archive-aware menus, and automatic restore when a new message is sent; the full archive remains searchable, grouped, and filterable by project, and the open web thread can be archived from the command palette or a rebindable shortcut (default mod+shift+E).
