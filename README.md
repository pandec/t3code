...
- **Model filter (mobile)** — the mobile thread-list filter menu adds a "Model" section alongside Environment and Project, listing the models the visible threads actually run on (labelled from the environment's catalog). The filters compose, a pin that no longer matches any thread falls back to "all models", and each thread row now leads its subtitle with the provider's mark.
