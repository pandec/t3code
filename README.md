...
- **Provider usage meter** — live, server-owned Claude Code and Codex subscription-quota windows with local reset times, retained per configured account and shared across threads. The web popover lists every enabled account for the thread's provider with independently redacted email, freshness, and on-demand refresh; mobile mirrors the multi-account list. After automatic rate-limit failover, both clients follow the session's live account rather than the originally picked one. Claude reports its per-window percentages through the same data as `/usage`; Codex renders whichever windows it reports. Optional warning/critical alerts and their thresholds are configurable on web.
