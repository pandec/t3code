...
- **Archived-thread search, grouping & project filtering** — archive chats from Sidebar V2 thread menus or archive the open web thread from the command palette or a rebindable shortcut, then search, group, and filter them by project, with direct scoped access from project menus and persisted repository identity so grouping survives project changes.
