...
- **Model filter (mobile)** — the mobile thread-list filter menu adds a "Model" section alongside Environment and Project (once more than one model is in play), listing the models your live threads run on, labelled from the environment's catalog. The filters compose, a pin that no longer matches any thread falls back to "all models", and grouped thread rows now show the provider's mark on the subtitle line.
