...
- **Claude shadow config dirs (multi-account)** — a Claude instance can pair a shared config dir with an account-specific "shadow" dir (like Codex shadow homes): credentials, MCP server registrations, and per-project prompt history stay per-account (they live in `.claude.json` / the credential store), while session transcripts, skills, agents, and commands — plus global settings and `CLAUDE.md` when present — are shared through materialized symlinks. Instances differing only by shadow dir form one continuation group, so a live thread can switch between two Claude subscriptions mid-conversation and resume the same session.
