...
- **Message queueing and steering** — on web, Enter during a running turn steers at the agent's next step after a five-second recall window; “Queue for later” holds the message until the turn completes, and ArrowUp recalls the newest queued message into an otherwise empty composer. The visible outbox on web and mobile also lets queued messages be steered, edited, or deleted; native mobile continues to queue new messages by default. A queued message whose images no longer resolve offers a recovery action instead of failing silently.
