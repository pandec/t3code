...
- **Session-handover CLI primitives** — inspect and import Claude/Codex transcripts (including standard worktrees, explicit model options, and an explicit thread title), create model-specific threads, and archive handed-off source threads. Transcript identity comes from the records themselves, so a file renamed in transit still imports, and history past the import cap is trimmed with a warning instead of rejected.
