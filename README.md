...
- **Automatic rate-limit failover** — a provider instance can name a same-driver sibling as its "Failover instance" in Settings. When the instance hits its subscription usage limit (a rejected rate-limit report from the CLI, or a turn failing with a rate-limit error), new turns route to the sibling until the limit lifts, then return. Live threads follow only when both instances share session state (one continuation group, e.g. the shadow config dir setup); each switch is announced in the thread's work log. Detection is provider-reported for Claude; other drivers mark an instance limited when a turn fails with a rate-limit error.
