...
- **Claude shadow config dirs (multi-account)** — a Claude instance can pair a shared config dir with an account-specific "shadow" dir (like Codex shadow homes): credentials stay per-account while session transcripts, skills, agents, and settings are shared through materialized symlinks. Instances differing only by shadow dir form one continuation group, so a live thread can switch between two Claude subscriptions mid-conversation and resume the same session.
